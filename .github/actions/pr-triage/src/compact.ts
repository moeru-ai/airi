import type { PullRequest, PullRequestFile } from './github'
...
type SelectedDiff = PullRequestFile | { omittedFiles: number }

function truncateMiddle(text: string, byteLimit: number, unit = 'lines'): string {
  ...
  const marker = `... ${Math.max(1, right - left + 1)} ${unit} truncated ...`
...
function selectDiffFiles(files: PullRequestFile[]): SelectedDiff[] {
  if (files.length <= 20)
    return files

  return [
    ...files.slice(0, 10),
    { omittedFiles: files.length - 20 },
    ...files.slice(-10),
  ]
}

/**
 * Builds an untrusted PR context that always fits within the configured UTF-8
 * byte budget when combined with the trusted system prompt and tag catalog.
 */
export function buildPrompt({
  ...
  const existingLabels = pr.labels
    .flatMap(label => typeof label === 'string' ? [label] : label.name ? [label.name] : [])
    .join(', ')
  const metadata = [
    `Number: #${pr.number}`,
    `Title: ${pr.title}`,
    ...
    `Reported files: ${pr.changed_files}`,
  ...
  const selectedCount = selected.filter(file => !('omittedFiles' in file)).length
  ...
  const diffBudget = Math.max(
    800,
    maxBytes - systemBytes - metadataBytes - manifestBytes - 1200,
  )
  const perFileBudget = Math.min(
    ...
    Math.max(
      180,
      selectedCount > 0
        ? Math.floor((diffBudget - selectedCount * 180) / selectedCount)
        : 180,
    ),
  )

  const diffParts = selected.map((file) => {
    if ('omittedFiles' in file)
      return `... ${file.omittedFiles} files truncated ...`
