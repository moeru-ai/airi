...
`server/apps/api` 提供业务 API；独立的 `server/apps/auth` 负责认证/OIDC。两者是不同 workspace app，不共享 composition root 或运行时依赖图。整体模式是：
...
- `server/apps/api/src/main.ts` → `server/apps/api/src/server.ts` → `server/apps/api/src/app.ts`
- `server/apps/auth/src/main.ts` → `server/apps/auth/src/server.ts`
...
环境 schema 也按 app 分开：`server/apps/api/src/libs/env.ts` 只定义 API 配置，
`server/apps/auth/src/env.ts` 只定义 Auth 配置。共享的
