...
- [`airi-server-overview-cloud.json`](https://github.com/proj-airi/airi-railway/blob/main/otel/grafana/dashboards/airi-server-overview-cloud.json)
...
| Server TTS metadata | `server/apps/api/src/routes/openai/v1/route.test.ts` 与 `server/apps/api/src/routes/audio-speech-ws/route.test.ts` 覆盖 REST / WS TTS `voice_id`、`voice_type`、`voice_pack_id` metadata |
| Grafana product row | `proj-airi/airi-railway/otel/grafana/dashboards/build.test.ts` 覆盖 Product Analytics panels、layout references、PromQL 不包含 high-cardinality voice / user fields |
