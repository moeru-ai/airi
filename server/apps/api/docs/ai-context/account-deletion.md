...
| 决策点 | 选择 | 理由 |
|---|---|---|
| `server/packages/auth-shared/src/schema.ts` | **不动** | `server/apps/auth` 的 better-auth `auth:generate` 自动产物。修改会被下次生成覆盖 |
...
```ts
// server/apps/api/src/services/domain/user-deletion/types.ts
...
业务 handlers 装配在 `server/apps/api/src/app.ts`；Auth adapter 装配在 `server/apps/auth/src/server.ts`。不分 transaction：每个 service 方法自己管 db/外部调用，**Stripe 这种没法 rollback 的副作用必须最先做**（priority 最小），失败会沿内部 HTTP 返回并中止 better-auth 的 user 删除，用户重试即可。

## 加新业务模块的步骤

1. 在该 service 加 `async deleteAllForUser(userId: string)` 方法
2. 在 `server/apps/api/src/app.ts` 的 `userDeletionService` build 里加一行 `service.register({...})`
3. 完成
...
- `server/apps/api/src/services/domain/flux.ts` — getBalance / readBalance
- `server/apps/api/src/services/domain/characters.ts` — listCharacters
- `server/apps/api/src/services/domain/providers.ts` — listProviderConfigs
- `server/apps/api/src/services/domain/chats.ts` — listChats / listMessages
- `server/apps/api/src/services/domain/billing/billing-service.ts` — invoice / sub 查询
...
- 业务表 schema: `server/apps/api/src/schemas/{flux,flux-transaction,stripe,characters,user-character,providers,chats}.ts`
- Auth schema (不改): `server/packages/auth-shared/src/schema.ts`
- Auth 配置: `server/apps/auth/src/auth.ts` (extend with `user.deleteUser`)
- Email service: `server/apps/auth/src/email.ts` (extend interface + Resend impl)
- Auth-to-resource-API boundary: `server/apps/auth/src/resource-api.ts`
- API internal boundary: `server/apps/api/src/routes/internal-auth.ts`
- Deletion scheduler: `server/apps/api/src/services/domain/user-deletion/` (registry only, no domain logic)
- 各 service 自己的 `deleteAllForUser`: `server/apps/api/src/services/{characters,chats,flux,providers,stripe}.ts`
