...
- `server/apps/auth/src/auth.ts` 启用 `admin({ adminRoles: ['admin'] })`。它给 `user` 表加 `role / banned / banReason / banExpires`，给 `session` 加 `impersonatedBy`（schema 位于 `server/packages/auth-shared/src/schema.ts`，字段名与插件一致，迁移 `drizzle/0013_naive_groot.sql`）。
...
- `server/apps/auth/src/auth.ts` — `admin()` 插件 + `disabledPaths`
- `server/packages/auth-shared/src/schema.ts` — user/session 上的 admin 插件字段
- `server/apps/api/src/middlewares/admin-guard.ts` — role-based `adminGuard`
- `server/packages/auth-shared/src/session.ts` — `isUserBannedNow` 共享 principal policy
- `server/apps/api/src/libs/request-auth.ts` — API 的本地 JWT 验签 + 热路径封禁闸
- `server/apps/auth/src/routes.ts` — `/oauth2/userinfo` 封禁 guard
- `server/apps/api/src/routes/admin/users/index.ts` — `POST /balance`（自建）
- `server/apps/api/src/services/domain/admin/users/index.ts` — `setBalance` 编排
- `server/apps/api/src/services/domain/billing/billing-service.ts` — `setFlux`
