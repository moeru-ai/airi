...
`server/apps/api` 的计费链：**Postgres 是唯一账本真相源，所有余额写操作（debit / credit）和 ledger 行写入都在同一个 DB 事务里完成**。Redis 只承担余额读缓存。不再使用 Redis Stream / 后台 consumer 处理计费副作用。
...
业务 API 使用独立入口 `server/apps/api/src/main.ts`，且没有任何"常驻后台 loop"或"fire-and-forget 异步任务"。所有写路径（包括 admin flux grant）都在请求线程内完成；多实例安全靠 `(userId, requestId)` 幂等索引。`server/apps/auth` 是另一独立应用，不参与 billing runtime。详见 [`workers-and-runtime.md`](workers-and-runtime.md)。
