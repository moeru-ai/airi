...
- `server/packages/auth-shared/src/schema.ts`

说明：

- `server/apps/auth` 的 `better-auth` 直接使用这组表
- 由 `pnpm -F @proj-airi/auth-server auth:generate` 自动产物，手改会被覆盖
- `server/apps/api` 只读取 user authorization projection，不加载 Better Auth runtime
