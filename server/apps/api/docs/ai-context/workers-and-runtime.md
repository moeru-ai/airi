...
API 入口：`server/apps/api/src/main.ts`。Auth server 使用独立 package 的
`server/apps/auth/src/main.ts`，不经过 role CLI，也不会加载 API
composition root。
...
这些变量只由 `server/apps/auth/src/env.ts` 解析，API 的
`server/apps/api/src/libs/env.ts` 不会读取或暴露它们。Auth server 另使用
...
`server/apps/auth/src/otel/gauges`，不会随 API runtime 加载。
