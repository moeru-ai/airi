...
- `auth-service-extraction.md`
  - `server/apps/auth` package、Caddy 路由、internal contract 与 migration ownership
...
- `server/apps/api/src/app.ts` 是唯一的 API 应用装配入口。
- 业务 API 采用 `Hono + injeca + Drizzle + Redis`；Better Auth 仅存在于独立的 `server/apps/auth`。
