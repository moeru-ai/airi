# Email auth via Resend (server/apps/auth + apps/ui-server-auth)
...
1. 接入 **Resend** 作为 `server/apps/auth` 的统一邮件发送 service。
...
- `server/apps/auth/src/email.ts`：统一 `EmailService` 接口（`sendVerification` / `sendPasswordReset` / `sendMagicLink` / `sendChangeEmail`），每个方法对应一个 HTML + plaintext 模板。
- `server/apps/auth/src/auth.ts`：装上 4 个 callback；启用 `requireEmailVerification: true`；加载 `magicLink` plugin。
- `server/apps/auth/src/env.ts`：定义 `RESEND_API_KEY`、`RESEND_FROM_EMAIL`、`RESEND_FROM_NAME` 与 `AUTH_UI_URL`。
- `server/apps/auth/src/server.ts`：把 `EmailService` 通过 `injeca` 装配，注入到 `auth` provider。
...
- **Verify / reset 链接 URL**：链接落地页不放 `server/apps/auth`，而是放独立部署的 `apps/ui-server-auth`。Auth server 的 public issuer 由 `PUBLIC_URL` 定义，UI base 由 `AUTH_UI_URL` 定义；两者通过 trustedOrigins 互信。链接组装规则：
...
- ui-server-auth 在 dev 下走 `http://localhost:5173`，与 `server/apps/auth` 不同源。Auth server 已在 `getAuthTrustedOrigins` 把 dev origin 加进来。
