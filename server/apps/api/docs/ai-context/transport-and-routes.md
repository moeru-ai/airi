...
- 运行入口：`server/apps/auth/src/main.ts`
- composition root：`server/apps/auth/src/server.ts`
- 路由入口：`server/apps/auth/src/routes.ts`
- token auth 辅助路由：`server/apps/auth/src/routes.ts`
- Electron 回调中继：`server/apps/auth/src/routes.ts`
- Better Auth 配置：`server/apps/auth/src/auth.ts`
- Bearer 解析：`server/apps/auth/src/routes.ts`
