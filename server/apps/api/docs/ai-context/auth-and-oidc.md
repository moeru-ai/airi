...
`server/apps/auth` 通过 `better-auth` 充当**用户认证后端**和 **OIDC Provider（Authorization Server）**，为 Web、Electron Desktop、Capacitor Mobile 三个客户端提供 Authorization Code + PKCE 登录流程。客户端持有 OIDC access token；`server/apps/api` 作为独立 resource server，通过 Auth server 的 JWKS 本地验签。
...
| 文件 | 职责 |
|------|------|
| `server/apps/auth/src/auth.ts` | better-auth 配置：社交 provider、OIDC provider 插件、trusted clients 种子数据、session/cookie 策略 |
| `server/apps/auth/src/routes.ts` | 所有鉴权路由的统一入口：sign-in 页、rate limiter、token auth 辅助路由、electron callback、well-known metadata、better-auth catch-all |
| `server/apps/auth/src/origin.ts` | Auth 可信来源配置 |
| `server/apps/auth/src/env.ts` | Auth/OIDC 环境变量定义（Valibot schema） |
| `server/apps/api/src/libs/request-auth.ts` | Resource API 鉴权：读取 Bearer JWT、通过 Auth JWKS 验签、加载授权 projection |
...
Resource API 通过 `server/apps/api/src/libs/request-auth.ts` 解析请求头：
...
Auth 路由集中在 `server/apps/auth/src/routes.ts`，通过 `.route('/', authRoutes)` 挂载到根路径。路由注册顺序很重要：
...
- 新增 OIDC client → `server/apps/auth/src/auth.ts` 的 `buildTrustedClientSeeds`
- 改登录页或 Auth UI redirect → `server/apps/auth/src/routes.ts`
- 改 Auth app 装配 → `server/apps/auth/src/server.ts`
- 改 trusted origins → `server/apps/auth/src/origin.ts`
- 改 API Bearer 鉴权解析 → `server/apps/api/src/libs/request-auth.ts`（JWT 本地验签，依赖 jose + Auth JWKS）
- 改 token auth 辅助路由 → `server/apps/auth/src/routes.ts`
- 改回调中继 → `server/apps/auth/src/routes.ts`
- 改 Auth 路由结构 → `server/apps/auth/src/routes.ts`
