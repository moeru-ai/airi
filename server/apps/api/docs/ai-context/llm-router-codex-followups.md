...
1. **Client abort signal not threaded into `llmRouter.route()`** — fixed in
   `server/apps/api/src/routes/openai/v1/index.ts`. `c.req.raw.signal` now flows
...
2. **Failed upstream response bodies not drained** — fixed in
   `server/apps/api/src/services/domain/llm-router/router.ts`. Every non-2xx fallback
...
3. **SSML voice attribute injection** — fixed in
   `server/apps/api/src/services/adapters/tts/azure.ts`. Voice id is
