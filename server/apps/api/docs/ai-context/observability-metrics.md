...
[airi-server-overview-cloud.json](https://github.com/proj-airi/airi-railway/blob/main/otel/grafana/dashboards/airi-server-overview-cloud.json) 由 [`build.ts`](https://github.com/proj-airi/airi-railway/blob/main/otel/grafana/dashboards/build.ts) 生成（**直接改 JSON 会在下次 regenerate 时被覆盖；改 build.ts**），在 `proj-airi/airi-railway` 运行 `pnpm dashboard:build` 重新生成。从上到下：
