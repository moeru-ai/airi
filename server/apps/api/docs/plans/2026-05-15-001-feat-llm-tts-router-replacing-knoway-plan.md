...
详见 origin 文档（server/apps/api/docs/brainstorms/2026-05-15-llm-router-replacement-requirements.md）。
...
| KTD-8 | 新增错误工厂 **`createBadGatewayError`**（502, `BAD_GATEWAY`）到 `server/apps/api/src/utils/error.ts` | 现有 helper 没 502 映射；走全局 `app.onError` 自动渲染 |
| KTD-9 | Voice catalog 用 **静态 JSON 提交仓库**（`server/apps/api/src/services/tts-adapters/voices/*.json`），不在运行时跨服务聚合 | origin D12 |
| KTD-10 | LLM/TTS 路由 logic **全部下沉到 `src/services/llm-router/`**，路由层只做 param validation + auth guard + 调 service + 处理响应；现有 `routes/openai/v1/index.ts` 的 TODO `:97-98` 同期解决 | server/apps/api/CLAUDE.md "Routes: thin — no business logic" |
...
| KTD-12 | `/livez` 和 `/readyz` 是**新路径**（K8s 风格，post-implementation 决定不保留 legacy `/health`），按现有 `httpInstrumentationMiddleware` 的探针排除规则同样跳过 | server/apps/api/src/app.ts:126-128 模式 |
...
**Files**:
- `server/apps/api/src/utils/envelope-crypto.ts` (NEW)
- `server/apps/api/src/utils/envelope-crypto.test.ts` (NEW)
- `server/apps/api/src/utils/error.ts` (MODIFY)
- `server/apps/api/src/services/config-kv.ts` (MODIFY — add `LLM_ROUTER_CONFIG` to `ConfigEntrySchemas`)
- `server/apps/api/src/libs/env.ts` (MODIFY — add `LLM_ROUTER_MASTER_KEY` env var)
- `server/apps/api/src/libs/env.test.ts` (MODIFY)
...
- `ConfigEntrySchemas` gets one new entry: `LLM_ROUTER_CONFIG: object({ llm: object({...}), tts: object({...}), defaults: object({...}) })`. Use `valibot` composition. **No `?? default` at call sites** — Valibot defaults live in the schema (server/apps/api/CLAUDE.md / config-and-naming-conventions).
- `createBadGatewayError(message, details?)` mirrors existing factories in `server/apps/api/src/utils/error.ts:18-69` — returns `ApiError(502, 'BAD_GATEWAY', message, details)`. Verify global `app.onError` (`server/apps/api/src/app.ts:160-182`) renders this with no further changes (it already pattern-matches `ApiError.statusCode`).
...
**Patterns to follow**:
- Error helpers: `server/apps/api/src/utils/error.ts:18-69` (existing factories)
- Valibot composite: `server/apps/api/src/services/config-kv.ts` `STRIPE_PAYMENT_METHOD_OPTIONS` precedent
- Env vars: `server/apps/api/src/libs/env.ts` existing schema definition pattern
...
- `pnpm exec vitest run server/apps/api/src/utils/envelope-crypto.test.ts` green
- `pnpm exec vitest run server/apps/api/src/libs/env.test.ts` green
...
**Files**:
- `server/apps/api/src/utils/observability.ts` (MODIFY — new `AIRI_ATTR_GEN_AI_GATEWAY_*` constants and `METRIC_AIRI_GEN_AI_GATEWAY_*` constants)
- `server/apps/api/src/otel/index.ts` (MODIFY — new `GatewayMetrics` bundle interface, factory `createGatewayMetrics`, prime list update)
- `server/apps/api/docs/ai-context/observability-metrics.md` (MODIFY)
- `server/apps/api/docs/ai-context/observability-conventions.md` (MODIFY — declare canonical `gen_ai.system` values: `openrouter`, `azure.speech`, `dashscope.cosyvoice`, `volcengine.tts`)
- `packages/server-shared/src/observability.ts` (MODIFY — if shared constants live here per learnings report)
- `server/apps/api/src/otel/index.test.ts` (MODIFY/NEW — prime list assertion)
...
- `OtelInstance` interface gains `gateway: GatewayMetrics | null` field; `null` when OTel is disabled (mirrors `genAi: GenAiMetrics | null` pattern at `server/apps/api/src/otel/index.ts:146-154`).
...
**Patterns to follow**:
- Attribute / metric constant convention: `server/apps/api/src/utils/observability.ts:6-90`
- Metric bundle factory: `server/apps/api/src/otel/index.ts:146-211+` (`createGenAiMetrics` precedent)
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/otel/index.test.ts` green
...
**Files**:
- `server/apps/api/src/services/llm-router/index.ts` (NEW)
- `server/apps/api/src/services/llm-router/router.ts` (NEW)
- `server/apps/api/src/services/llm-router/key-rotator.ts` (NEW)
- `server/apps/api/src/services/llm-router/config-loader.ts` (NEW)
- `server/apps/api/src/services/llm-router/error-mapping.ts` (NEW)
- `server/apps/api/src/services/llm-router/types.ts` (NEW)
- `server/apps/api/src/services/llm-router/router.test.ts` (NEW)
- `server/apps/api/src/services/llm-router/key-rotator.test.ts` (NEW)
- `server/apps/api/src/services/llm-router/config-loader.test.ts` (NEW)
- `server/apps/api/src/services/llm-router/error-mapping.test.ts` (NEW)
...
**Patterns to follow**:
- DI via factory pattern: `server/apps/api/src/services/billing/billing-service.ts` and other `create*Service` factories
- Fetch injection for tests: `server/apps/api/src/routes/openai/v1/route.test.ts:78-91` `globalThis.fetch = vi.fn(...)` pattern adapted to DI prop
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/services/llm-router/` green
...
**Goal**: Replace the inline upstream fetch logic in `server/apps/api/src/routes/openai/v1/index.ts` `handleCompletion` with a call to the new `LlmRouterService`. Preserve all existing billing / OTel / SSE streaming / first-token latency behavior. Address the TODO at `:97-98` (split mixed concerns).
...
**Files**:
- `server/apps/api/src/routes/openai/v1/index.ts` (MODIFY)
- `server/apps/api/src/routes/openai/v1/route.test.ts` (MODIFY)
- `server/apps/api/src/app.ts` (MODIFY — DI: inject `LlmRouterService` into `createV1CompletionsRoutes` factory)
...
**Patterns to follow**:
- Existing SSE streaming pipe pattern in `server/apps/api/src/routes/openai/v1/index.ts:194-330` — preserve exactly
- DI wiring: `server/apps/api/src/app.ts` `injeca.provide(...)` for `llmRouter` service + add to `AppDeps`
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/routes/openai/v1/route.test.ts` green
...
**Files**:
- `server/apps/api/src/services/tts-adapters/types.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/index.ts` (NEW — adapter registry by provider id)
- `server/apps/api/src/services/tts-adapters/azure.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/azure.test.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/dashscope-cosyvoice.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/dashscope-cosyvoice.test.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/volcengine.ts` (NEW)
- `server/apps/api/src/services/tts-adapters/volcengine.test.ts` (NEW)
- `server/apps/api/package.json` (MODIFY — add `unspeech` to `devDependencies` for type-only import of `Voice` / `VoiceFormat` / `VoiceLanguage`)
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/services/tts-adapters/` green
...
**Files**:
- `server/apps/api/src/services/tts-adapters/voices/azure.json` (NEW)
- `server/apps/api/src/services/tts-adapters/voices/dashscope-cosyvoice.json` (NEW)
- `server/apps/api/src/services/tts-adapters/voices/volcengine.json` (NEW)
- `server/apps/api/src/services/tts-adapters/azure.ts` (MODIFY — `getVoiceCatalog()` reads json file at module init)
- (same for dashscope-cosyvoice.ts, volcengine.ts)
- `server/apps/api/src/routes/openai/v1/index.ts` (MODIFY — replace `handleListVoices` with adapter-dispatch)
- `server/apps/api/src/routes/openai/v1/route.test.ts` (MODIFY — add tests)
- `server/apps/api/docs/ai-context/verifications/llm-router.md` (NEW — initial voices refresh runbook section)
...
- Removed: passthrough to `${baseUrl}audio/voices` (was knoway-dependent at `server/apps/api/src/routes/openai/v1/index.ts:441-475`).

**Patterns to follow**:
- Hono Response.json: existing usage at `server/apps/api/src/routes/openai/v1/index.ts:474`
- Type-only import: `import type { Voice } from 'unspeech'`
- ESM JSON imports: TypeScript 5+ + Node 22+ support; verify in `server/apps/api/tsconfig.json` (resolveJsonModule)
...
- (3) Empty `model` falls back to `env.DEFAULT_TTS_MODEL` (matches current behavior at `server/apps/api/src/routes/openai/v1/index.ts:441-446`).
...
**Files**:
- `server/apps/api/src/services/llm-router/config-loader.ts` (MODIFY — add `subscribeToInvalidations(redis)` wiring)
- `server/apps/api/src/utils/redis-keys.ts` (MODIFY — add `configKvInvalidateChannel()` helper)
- `server/apps/api/src/app.ts` (MODIFY — register `/livez`, `/readyz`, exclude both from `httpInstrumentationMiddleware`; wire config-loader to redis subscriber; admin endpoint for `set LLM_ROUTER_CONFIG` publishes invalidation)
- `server/apps/api/src/routes/admin/...` (MODIFY — if admin set endpoint exists for configKV; publish on write)
- `server/apps/api/src/app.test.ts` (NEW — health endpoint tests)
- `server/apps/api/docs/ai-context/redis-boundaries-and-pubsub.md` (MODIFY — declare `configkv:invalidate` channel)
...
- `/livez`: route returns `200 {status: 'live'}` always. No DB / Redis touch. Excluded from `httpInstrumentationMiddleware` (`server/apps/api/src/app.ts:126-128` pattern).
...
**Patterns to follow**:
- ioredis Pub/Sub: dedicated subscriber connection (search for existing pubsub usage in `server/apps/api` — `redis-boundaries-and-pubsub.md` references this)
- redis-keys helper: `server/apps/api/src/utils/redis-keys.ts:11+` `redisKeyFrom` pattern
- Health route skip: existing pattern at `server/apps/api/src/app.ts:126-128`
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/app.test.ts` green
...
**Goal**: 解决 doc review P0 blocker —— 现有 `server/apps/api/src/routes/admin/` 只有 `flux-grants` 和 `flux-grant-batches`，**没有** configKV.set 的 HTTP 入口。本 unit 新建一个 admin endpoint 承接 `LLM_ROUTER_CONFIG` 的读 / 写 / 部分更新，并在写入后发 Pub/Sub 通知用以触发 U7 的 invalidation。同时实现 R16a 角色模型决策（在此 endpoint 落地）+ 写入审计日志 + 原子性约束。
...
**Files**:
- `server/apps/api/src/routes/admin/llm-router-config.ts` (NEW)
- `server/apps/api/src/routes/admin/llm-router-config.test.ts` (NEW)
- `server/apps/api/src/app.ts` (MODIFY — mount admin route under `/api/admin/llm-router-config`)
- `server/apps/api/src/services/llm-router/config-loader.ts` (MODIFY — service 内部 `setLlmRouterConfig(value)` 包装 configKV.set + 加密 + publish 三步)
...
**Patterns to follow**:
- 现有 admin route 风格：`server/apps/api/src/routes/admin/flux-grants` (HTTP shape + auth guard)
- ADMIN_EMAILS 检查：`server/apps/api/src/app.ts:244` 现有 pattern
...
**Verification**:
- `pnpm -F @proj-airi/server typecheck` passes
- `pnpm exec vitest run server/apps/api/src/routes/admin/llm-router-config.test.ts` green
...
**Files**:
- `proj-airi/airi-railway/otel/grafana/dashboards/build.ts` (MODIFY)
- `proj-airi/airi-railway/otel/grafana/dashboards/airi-server-overview-cloud.json` (MODIFY)
- `server/apps/api/src/app.ts` (MODIFY — final DI wiring; remove `GATEWAY_BASE_URL` consumers)
- `server/apps/api/src/libs/env.ts` (MODIFY — remove `GATEWAY_BASE_URL`)
- `server/apps/api/src/libs/env.test.ts` (MODIFY)
- `server/apps/api/scripts/verify-router-config.ts` (NEW — operator script referenced in U1 Migration step; decrypts current `LLM_ROUTER_CONFIG` against `LLM_ROUTER_MASTER_KEY` to validate boot-time correctness)
- `server/apps/api/src/scripts/otel/llm-router-smoke.ts` (NEW — new smoke fixture that produces traces tagged with `airi.gen_ai.gateway.*` attrs; **note**: the previously-referenced `server/apps/api/src/scripts/otel/smoke.ts` does not exist — the actual existing file is `ws-smoke.ts` for WebSocket smoke; gateway-specific smoke is new work)
- `server/apps/api/docs/ai-context/transport-and-routes.md` (MODIFY — route → service mapping update; `/livez` + `/readyz` documented; legacy `/health` removed)
- `server/apps/api/docs/ai-context/observability-conventions.md` (MODIFY)
- `server/apps/api/docs/ai-context/verifications/llm-router.md` (FINALIZE — full verification with real evidence)
- (Possibly) `server/apps/api/scripts/...` (NEW — knoway compose retention policy doc / data-driven trigger criteria)
...
- DI wiring: extend `AppDeps` interface (`server/apps/api/src/app.ts:70-88`) with `llmRouter` field. Register via `injeca.provide('services:llmRouter', { dependsOn: ['services:configKV', 'libs:redis', 'otel'], build: ... })`. Thread into `createV1CompletionsRoutes` factory.
- `GATEWAY_BASE_URL` removal: delete from env schema; verify no remaining consumers via grep — current consumers per existing brainstorm context: `server/apps/api/src/libs/env.ts`, `server/apps/api/src/libs/env.test.ts`, `server/apps/api/src/routes/openai/v1/index.ts`, `server/apps/api/src/routes/openai/v1/route.test.ts`, `server/apps/api/src/scripts/otel/smoke.ts`. Update each.
- Verification doc (`server/apps/api/docs/ai-context/verifications/llm-router.md`): follow AGENTS.md template — for each user path (chat completions happy / chat completions fallback / TTS speech happy / voices listing / livez / readyz), include scenario / command / expected output / actual output (curl response snippets) / environment (commit SHA + deploy env) / last verified date.
...
**Patterns to follow**:
- DI wiring: existing `injeca.provide` blocks in `server/apps/api/src/app.ts:280-487`
- Verification doc format: AGENTS.md template + existing files under `server/apps/api/docs/ai-context/verifications/`
...
- (3) Smoke test (new `server/apps/api/src/scripts/otel/llm-router-smoke.ts`): produces traces tagged with `airi.gen_ai.gateway.*` attrs.
...
- Grafana dashboard rendering: load JSON into local Grafana via `proj-airi/airi-railway/otel/docker-compose.yaml`; visually confirm 3 new panels appear
...
| Surface | Impact |
|---|---|
| `server/apps/api` request thread | New synchronous path: configKV cache check → router service → upstream(s) → response. No new background work. |
| `server/apps/api` startup | New env var `LLM_ROUTER_MASTER_KEY` required. configKV must contain `LLM_ROUTER_CONFIG` before requests succeed (otherwise router throws `CONFIG_NOT_SET` per existing pattern). |
...
| Other services in repo | None. `server/apps/api` is the only consumer of `LLM_ROUTER_CONFIG`. |
...
| SSE first-byte race: fallback decision made after first byte already streamed | Low | High | Explicit pre-first-byte gate in router.ts (U3 test #9); mirrors existing pattern at `server/apps/api/src/routes/openai/v1/index.ts:194` |
...
Verification doc lives at `server/apps/api/docs/ai-context/verifications/llm-router.md`. Covers these user paths (per AGENTS.md format — scenario + command + expected + actual + environment + last-verified):
...
- Requirements doc: `server/apps/api/docs/brainstorms/2026-05-15-llm-router-replacement-requirements.md`
...
- AGENTS.md / `server/apps/api/CLAUDE.md` constraints honored:
