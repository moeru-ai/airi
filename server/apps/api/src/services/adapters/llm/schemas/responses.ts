...
const file = v.pipe(v.strictObject({ ...base.vInputFileContentParam.entries, file_id: v.nullish(v.string()) }), v.check(part => Boolean(part.file_id || part.file_data || part.file_url), 'A file needs an ID, inline data or a URL'))
const image = v.pipe(v.strictObject({ ...base.vInputImageContentParamAutoParam.entries, file_id: v.nullish(v.string()), detail: v.nullish(v.picklist(['auto', 'low', 'high', 'original'])) }), v.check(part => Boolean(part.file_id || part.image_url), 'An image needs an ID or URL'))
...
const functionParametersSchema = v.pipe(
  v.looseObject({
    ...
    properties: v.optional(v.pipe(jsonSchemaObject, v.check(value => Object.values(value).every(node => v.safeParse(jsonSchemaNode, node).success), 'JSON Schema properties must contain object or boolean schemas'))),
    required: v.optional(v.array(v.string())),
  }),
  v.check(schema => schema.required == null || new Set(schema.required).size === schema.required.length, 'JSON Schema required fields must be unique'),
  v.check(schema => schema.required == null || schema.required.every(name => schema.properties != null && Object.hasOwn(schema.properties, name)), 'JSON Schema required fields must exist in properties'),
)
...
const message = v.union([
  v.strictObject({ ...base.vUserMessageItemParam.entries, role: v.literal('user'), content: v.union([v.pipe(v.string(), v.maxLength(10485760)), v.array(inputPart)]) }),
  v.strictObject({ ...base.vSystemMessageItemParam.entries, role: v.literal('system') }),
  v.strictObject({ ...base.vDeveloperMessageItemParam.entries, role: v.literal('developer') }),
  v.strictObject({ ...base.vAssistantMessageItemParam.entries, role: v.literal('assistant'), content: v.union([v.pipe(v.string(), v.maxLength(10485760)), v.array(outputPart)]), phase: v.optional(v.nullable(base.vAssistantMessageItemParam.entries.phase.wrapped)) }),
])
...
// NOTICE:
// The live Responses API can omit the URL from replayed find-in-page actions.
// The generated OpenAI SDK contract still marks this field as required.
// Source/context: https://github.com/openai/openai-python/issues/3788
// Remove this override when the live API and published contract require the URL.
const findInPageActionSchema = v.strictObject({ type: v.literal('find_in_page'), url: v.optional(v.nullable(v.string())), pattern: v.string() })
...
export const responseItemSchema = v.union([
  ...
  v.strictObject({ ...base.vReasoningItemParam.entries, content: v.nullish(v.array(v.strictObject({ type: v.literal('reasoning_text'), text: v.string() }))), status: v.nullish(base.vFunctionCallStatus) }),
  base.vFunctionCallItemParam,
  v.strictObject({ ...base.vFunctionCallOutputItemParam.entries, output: v.union([v.pipe(v.string(), v.maxLength(10485760)), v.pipe(v.array(inputPart), v.minLength(1))]) }),
  webSearchCallSchema,
])
