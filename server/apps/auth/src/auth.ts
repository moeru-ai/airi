...
import { email as emailAddress, nonEmpty, object, parse, pipe, string, transform } from 'valibot'
...
const ChangeEmailPolicyBodySchema = object({
  newEmail: pipe(
    string(),
...
function parseChangeEmailPolicyBody(body: unknown) {
  try {
    return parse(ChangeEmailPolicyBodySchema, body)
  }
  catch {
...
export function createAuth(
  ...
  const auth = betterAuth({
    ...
    user: {
      ...
      changeEmail: {
        enabled: true,
        // Placeholder accounts are the only authenticated unverified accounts
        // that the server creates. Better Auth updates these accounts first,
        // then sends an optional verification email to the new address.
    ...
    hooks: {
      before: createAuthMiddleware(async (ctx) => {
        if (ctx.path === '/change-email') {
          const { newEmail } = parseChangeEmailPolicyBody(ctx.body)
