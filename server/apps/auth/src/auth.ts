  ...
  const auth = betterAuth({
    ...
    user: {
      ...
      deleteUser: {
        ...
        enabled: true,
        async sendDeleteAccountVerification({ url, user }) {
...
export async function ensureDynamicFirstPartyRedirectUri(
  ...
  let normalizedRedirectUri: null | string = null
...
function buildTrustedElectronRedirectUri(request: Request, redirectUri: string): null | string {
  try {
...
function buildTrustedWebRedirectUri(redirectUri: string, additionalTrustedOrigins: readonly string[]): null | string {
  try {
    const parsed = new URL(redirectUri)
...
function createAppleProviderConfig(
  env: Pick<AuthEnv, 'AUTH_APPLE_APP_BUNDLE_IDENTIFIERS' | 'AUTH_APPLE_CLIENT_ID' | 'AUTH_APPLE_KEY_ID' | 'AUTH_APPLE_PRIVATE_KEY_PEM' | 'AUTH_APPLE_TEAM_ID'>,
) {
