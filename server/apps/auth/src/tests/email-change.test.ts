...
describe('native email change', () => {
  ...
  it('updates a migrated placeholder email without waiting for verification', async () => {
    const fixture = await createVerifiedFixture()
  ...
  it('updates another authenticated unverified email through the native flow', async () => {
    const fixture = await createVerifiedFixture()
    await fixture.db.update(user)
  ...
  it('rejects a malformed destination before any email is sent', async () => {
    const fixture = await createVerifiedFixture()
    const response = await fixture.auth.handler(new Request(`${BASE_URL}/api/auth/change-email`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        'cookie': fixture.cookie,
