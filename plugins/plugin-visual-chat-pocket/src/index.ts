...
export interface PocketBridgeOptions {
  gatewayUrl: string
}

export class PocketVisualChatBridge {
  ...
  constructor(private opts: PocketBridgeOptions) {
  ...
  async switchToPhoneCamera(): Promise<void> {
    if (!this.sessionId)
      return
    await this.client.switchSource(this.sessionId, undefined, 'phone-camera')
  }
