...
export const sparkNotifyCommandGuidanceSchema = z
  .object({
    ...
    persona: z
      ...
      .describe('Optional persona controls for the receiver.'),
...
export const sparkNotifyCommandItemSchema = z
  .object({
    ...
    interrupt: z
      ...
      .describe('Interrupt type: force, soft, or false (no interrupt).'),
    priority: z
      .union([z.enum(['critical', 'high', 'normal', 'low']), z.null()])
      .describe('Semantic priority of the command.'),
    intent: z
      .union([z.enum(['plan', 'proposal', 'action', 'pause', 'resume', 'reroute', 'context']), z.null()])
      .describe('Intent of the command.'),
    ...
    guidance: z
      ...
      .describe('Guidance for the sub-agent on how to interpret and execute the command.'),
  })
  .strict()

export const sparkNotifyCommandSchema = z
  .object({
    commands: z
      .array(sparkNotifyCommandItemSchema)
      .describe('List of commands to issue to sub-agents. Empty array can be used for zero commands.'),
  })
  .strict()
