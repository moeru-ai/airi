...
export class UpstashRedisShortTermMemoryProvider extends BaseShortTermMemoryProvider {
  ...
  private static createClient(options: UpstashRedisShortTermMemoryOptions): Redis {
    const url = options.url ?? env.UPSTASH_REDIS_REST_URL
    const token = options.token ?? env.UPSTASH_REDIS_REST_TOKEN

    if (!url || !token) {
      throw new Error('Upstash Redis configuration is missing. Provide url/token or set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN.')
    }
