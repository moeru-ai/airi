...
export const useModelStore = defineStore('modelStore', () => {
  ...
  // === Lighting ===
  const defaultHemisphereLightSettings = { intensity: 2.02, color: '#fffbf5' as HexColor }
  ...
  // Per-environment directional light presets.
  const directionalLightSettingsByEnv = useLocalStorage<Record<'hemisphere' | 'skyBox', { intensity: number, color: HexColor }>>(
    ...
    {
      hemisphere: defaultHemisphereLightSettings,
      skyBox: { intensity: 1.2, color: '#ffffff' },
    },
  )
  const directionalLightIntensity = computed(() => directionalLightSettingsByEnv.value[envSelect.value]?.intensity ?? defaultHemisphereLightSettings.intensity)
  const directionalLightColor = computed(() => directionalLightSettingsByEnv.value[envSelect.value]?.color ?? defaultHemisphereLightSettings.color)
