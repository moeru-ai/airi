...
describe('live2d zip loader with the combined cubism 2 and cubism 4 runtime', () => {
  ...
  it.each(['zip', 'directory'] as const)('loads uppercase loose Cubism 4 files through the combined runtime from a %s', async (source) => {
    const runtime = await import('pixi-live2d-display')
    const { configureLive2DLoaders } = await import('./live2d-zip-loader')
    configureLive2DLoaders(runtime)
    const files = [
      fileWithRelativePath(new Uint8Array([77, 79, 67, 51]), 'MODEL.MOC3', 'avatar/MODEL.MOC3'),
      fileWithRelativePath(new Uint8Array([1, 2, 3]), 'TEXTURE.PNG', 'avatar/TEXTURE.PNG'),
    ]
    const zip = new JSZip()
    for (const file of files)
      zip.file(file.webkitRelativePath, await file.arrayBuffer())

    const settings = source === 'zip'
      ? await runtime.ZipLoader.createSettings(zip)
      : await runtime.FileLoader.createSettings(files)

    // ROOT CAUSE:
    // The standalone Cubism 4 bundle owns a different ModelSettings class.
    // Its generated settings fail the combined runtime's instanceof check,
    // so loose archives fail with "Unknown settings format" after parsing.
    // Create settings through the selected runtime for both ZIP and OPFS input.
    expect(settings).toBeInstanceOf(runtime.ModelSettings)
    expect(runtime.Live2DFactory.findRuntime(settings)).toBeDefined()
    expect(settings.textures).toEqual(['avatar/TEXTURE.PNG'])
