...
export const useExpressionStore = defineStore('live2d-expressions', () => {
  ...
  function toggle(name: string, duration?: number): ExpressionToolResult {
    ...
    // Direct parameter toggle: flip between the blend's neutral value and the
    // expression's target. Neutral is `defaultValue`, not `modelDefault`: an Add
    // parameter rests at 0 and a Multiply one at 1 regardless of the model's own
    // default, so toggling against `modelDefault` never returned Add/Multiply
    // parameters to a no-op state.
