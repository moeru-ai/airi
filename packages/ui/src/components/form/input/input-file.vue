...
const fileNames = computed(() => {
  ...
  if (!files.length) return props.placeholder
...
<template>
  ...
  >
    <input type="file" :accept="accept" :multiple="multiple" :class="['hidden']" @change="onFileChange" />

    <div :class="['i-solar:upload-square-line-duotone h-5 w-5 shrink-0 text-neutral-500 dark:text-neutral-400']" />

    <div :class="['min-w-0 flex-1 truncate text-sm text-neutral-600 dark:text-neutral-300']" :title="fileNames">
      {{ fileNames }}
    </div>
