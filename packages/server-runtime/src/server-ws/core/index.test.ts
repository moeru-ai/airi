...
import { createConsumerOrchestrator, selectConsumerPeerId } from '.'
...
describe('server-ws consumer selection', () => {
  ...
  it('preserves sticky assignment for the same sticky key', () => {
    ...
    const delivery = {
      mode: 'consumer-group' as const,
      group: 'workers',
      selection: 'sticky' as const,
      stickyKey: 'job-1',
    }
    const candidates = [
  ...
  it('preserves round-robin cursor per event and group', () => {
    ...
    expect(
      selectConsumerPeerId({ eventType: 'event:test', fromPeerId: 'sender', delivery, candidates, roundRobinCursor }),
    ).toBe('a')
    expect(
      selectConsumerPeerId({ eventType: 'event:test', fromPeerId: 'sender', delivery, candidates, roundRobinCursor }),
    ).toBe('b')
  ...
  it('resets round-robin cursor when group membership changes', () => {
    ...
    expect(
      registry.select({
        ...
        candidates: registry
          .listFor({ event: 'event:test', mode: 'consumer-group', group: 'workers' })
          .map((entry) => ({
            peerId: entry.peerId,
