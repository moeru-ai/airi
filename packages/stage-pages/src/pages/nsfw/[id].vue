...
const accessBadges = computed(() => {
  ...
  return [
    character.value.nsfwLevel,
    character.value.relationshipMode,
    character.value.visibility,
  ].filter(Boolean)
})
  ...
  <div class="min-h-screen px-4 py-8 md:px-8">
    <div mx-auto max-w-6xl>
      ...
      <div v-if="loading" class="rounded-[2rem] bg-white p-10 text-center shadow-sm ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800">
        Loading NSFW profile...
      </div>

      <div v-else-if="accessDenied" class="rounded-[2rem] bg-white p-10 text-center shadow-sm ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800">
        NSFW access is disabled for this account.
      </div>

      <div v-else-if="failed || !character || !i18n || !character.nsfwEnabled || character.nsfwLevel === 'none'" class="rounded-[2rem] bg-white p-10 text-center shadow-sm ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800">
        NSFW character not found.
      </div>
          ...
          <div class="relative z-1 min-h-[34rem] flex flex-col justify-end gap-5 p-6 md:p-8">
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              ...
              <div v-if="character.personaProfile?.speakingStyle" class="rounded-2xl bg-white/10 px-4 py-3 backdrop-blur">
                ...
                <div class="mt-1 text-sm font-medium">
                  {{ character.personaProfile?.speakingStyle }}
                </div>
              </div>
              <div class="rounded-2xl bg-white/10 px-4 py-3 backdrop-blur">
                <div class="text-[11px] text-white/60 tracking-[0.16em] uppercase">
        ...
        <section class="flex flex-col gap-6">
          ...
          <div class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800">
            ...
            <p v-else class="mt-3 text-sm text-neutral-500 dark:text-neutral-400">
              No starter messages configured.
            </p>
          </div>
