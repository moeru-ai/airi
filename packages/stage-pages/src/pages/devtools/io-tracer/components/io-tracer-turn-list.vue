  ...
  >
    ...
    >
      <div :class="['flex items-center justify-between mb-1']">
        <span :class="['text-xs font-mono font-medium']">#{{ turn.id.slice(0, 6) }}</span>
        <span
          :class="[
      ...
      <div v-if="turn.inputText" :class="['text-2.5 text-neutral-500 truncate mb-1']">
        {{ turn.inputText.slice(0, 50) }}{{ turn.inputText.length > 50 ? '...' : '' }}
      </div>

      <div v-if="getTtft(turn)" :class="['text-2.5 text-purple-500 mb-1']">TTFT: {{ formatMs(getTtft(turn)!) }}</div>
