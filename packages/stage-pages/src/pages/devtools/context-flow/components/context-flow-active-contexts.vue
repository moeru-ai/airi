...
const filteredBuckets = computed(() => {
  ...
  if (!query) return props.buckets

  return props.buckets.filter((bucket) => {
    const haystack = [
      bucket.sourceKey,
      ...bucket.messages.map((message) =>
        [message.text, message.contextId, message.id, message.lane, message.strategy].filter(Boolean).join(' '),
      ),
    ]
      .join(' ')
      .toLowerCase()
...
<template>
  <Section title="Active Context State" icon="i-solar:database-bold-duotone" inner-class="gap-3" :expand="true">
