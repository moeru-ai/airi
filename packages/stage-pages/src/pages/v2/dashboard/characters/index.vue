...
onMounted(() => {
  characterStore.ensureDefaultCharacters().catch(console.error)
})
...
const characters = computed(() => Array.from(characterStore.characters.values()).map((char) => {
  ...
  return {
    ...
    modelType: char.avatarModels?.[0]?.type,
  }
}))
</script>
  ...
  >
    <div>
      <div :class="['grid gap-3', 'sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5']">
        ...
        >
          <div :class="['relative overflow-hidden w-full h-65% rounded-2xl']">
            <img
          ...
          <div :class="['relative z-7 overflow-hidden h-35%']">
          ...
          <div :class="['absolute left-0 top-0 z-0 w-full h-full']">
            <div :class="['absolute inset-0 z-2 bg-white/70 backdrop-blur-md w-full h-full']" />
            <img
              :src="character.coverUrl"
              :alt="character.name"
              :class="[
