...
function buildReplyLanguageInstruction(locale: string) {
  const language = localeToLanguageName(locale)
  return `Reply language policy: default to ${language}. If the user explicitly asks to switch languages, follow the request.`
}
...
export const useAiriCardStore = defineStore('airi-card', () => {
  ...
  const replyLanguageInstruction = computed(() => {
    const locale = settingsGeneral.language || settingsGeneral.getLanguage()
    return buildReplyLanguageInstruction(locale)
  })
  ...
  return {
    ...
    systemPrompt: computed(() => {
      ...
      const components = [
        replyLanguageInstruction.value,
        card.systemPrompt,
        card.description,
        card.personality,
      ].filter(Boolean)
