...
function normalizePercentOption(value: string | number | null | undefined, name: string): number | undefined {
  ...
  if (typeof value !== 'string') throw new Error(`Voice Pack parameter "${name}" must be a number or percent string.`)

  const trimmed = value.trim()
  if (trimmed === '') return undefined

  const normalized = trimmed.endsWith('%') ? trimmed.slice(0, -1).trim() : trimmed
  if (normalized === '') throw new Error(`Voice Pack parameter "${name}" must be a number or percent string.`)

  const parsed = Number(normalized)
  if (!Number.isFinite(parsed)) throw new Error(`Voice Pack parameter "${name}" must be a number or percent string.`)
...
function assertSupportedVoicePackParams(params: VoicePackParams | undefined) {
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    if (!voicePackSupportedParams.has(key)) throw new Error(`Unsupported Voice Pack parameter "${key}".`)
  }
}
...
export const useSpeechStore = defineStore('speech', () => {
  ...
  function ensureStreamingDefaultModel() {
    ...
    const hasValidSelection =
      Boolean(activeSpeechModel.value) && streamingModels.some((m) => m.id === activeSpeechModel.value)
  ...
  function generateSSML(text: string, voice: VoiceInfo, providerConfig?: Record<string, unknown>): string {
    ...
    const prosody = {
      pitch: typeof pitch === 'number' ? toSignedPercent(pitch) : undefined,
      rate: typeof speed === 'number' ? (speed !== 1.0 ? `${speed}` : '1') : undefined,
      volume: typeof volume === 'number' ? toSignedPercent(volume) : undefined,
    }
  ...
  function resolveVoicePackSpeechInput(options: VoicePackSpeechInputOptions): VoicePackSpeechInput {
    ...
    if (!options.params && !options.voicePack) {
      return {
        input:
          options.forceSSML && options.supportsSSML
    ...
    if (speed != null) providerConfig.speed = speed
    ...
    if (options.voicePack) {
      providerConfig.extraBody = {
        ...
        voice_pack: {
          ...
          ...(needsProsody && options.supportsAdapterProsody ? { pitch, volume } : {}),
        },
      }
    ...
    if (pitch != null) ssmlConfig.pitch = pitch
    if (volume != null) ssmlConfig.volume = volume
