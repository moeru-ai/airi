...
function injectStrictToolUseGuidance(messages: Message[]): Message[] {
  ...
  const guidance: Message = {
    ...
    content: [
      ...
      '5. For real browser pages, Discord Web, OAuth flows, or iframe-heavy forms, prefer browser_dom_* tools for deterministic DOM reads/writes and verification. If the page needs freer exploration or multi-step web reasoning, you MAY call browser_agent_run to delegate the web task to the dedicated browser agent.',
