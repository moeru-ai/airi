...
export const providerElevenLabsSpeech = defineProvider<ElevenLabsConfig>({
  ...
  validators: {
    validateConfig: [
      ({ t }: { t: ComposerTranslation }) => ({
        ...
        validator: async (config: ElevenLabsConfig): Promise<ProviderValidationResult> => {
          ...
          return {
            errors,
            reason: errors.length > 0 ? errors.map(e => (e as Error).message).join(', ') : '',
  ...
  extraMethods: {
    ...
    async listVoices(config, provider): Promise<VoiceInfo[]> {
      const voiceProvider = provider as VoiceProviderWithExtraOptions<UnElevenLabsOptions>
