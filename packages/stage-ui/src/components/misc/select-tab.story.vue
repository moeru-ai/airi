...
<template>
  <Story title="SelectTab" group="misc">
    <template #controls>
      <ThemeColorsHueControl />
    </template>

    <Variant id="default" title="Mode selection">
      <div class="max-w-4xl w-full flex flex-col gap-4">
        <SelectTab v-model="focusMode" :options="focusOptions" />
        <p class="text-sm text-neutral-600 dark:text-neutral-300">Selected mode: {{ focusMode }}</p>
      </div>
    </Variant>

    <Variant id="with-descriptions" title="Sizing and descriptions">
      <div class="max-w-3xl w-full flex flex-col gap-4">
        <SelectTab v-model="quality" size="sm" :options="qualityOptions" />
        <p class="text-sm text-neutral-600 dark:text-neutral-300">Rendering quality: {{ quality }}</p>
      </div>
    </Variant>

    <Variant id="disabled-readonly" title="Disabled and readonly states">
      <div class="max-w-3xl w-full flex flex-col gap-4">
        <SelectTab v-model="locked" :options="lockedOptions" disabled />
        <SelectTab v-model="locked" :options="lockedOptions" readonly />
        <p class="text-sm text-neutral-600 dark:text-neutral-300">Locked selection mirrors both states: {{ locked }}</p>
      </div>
    </Variant>
  </Story>
</template>
