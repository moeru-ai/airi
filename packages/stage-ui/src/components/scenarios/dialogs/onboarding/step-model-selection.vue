...
const { activeModel, modelSearchQuery, providerModels, isLoadingActiveProviderModels, activeProviderModelError } =
...
<template>
  <div :class="['min-h-0 flex min-w-0 flex-1 flex-col gap-4']">
    ...
    <div class="min-h-0 flex flex-1 flex-col gap-4 overflow-hidden">
      <Alert v-if="providerModels.length === 0 && !isLoadingActiveProviderModels" type="error">
        <template #title>
      ...
      <RadioCardManySelect
        ...
        :custom-input-placeholder="
          t('settings.pages.modules.consciousness.sections.section.provider-model-selection.custom_model_placeholder')
        "
