...
// 2. 监听选中模型变化：更新高亮并自动跳转到对应分页
watch(() => props.selectedModel?.id, (modelId) => {
  highlightDisplayModelCard.value = modelId
  if (modelId) {
    const index = filteredModels.value.findIndex(m => m.id === modelId)
    if (index !== -1) {
...
<template>
  <div pt="4 sm:0" gap="4 sm:6" h-full flex flex-col>
    <div flex items-center gap-2>
      <div w-full flex-1 text-xl>Model Selector</div>
      ...
      <div>
        <DropdownMenuRoot>
          ...
          <DropdownMenuPortal>
            <DropdownMenuContent
              class="z-10000 max-w-45 rounded-lg bg-neutral-100/50 p-0.5 shadow-md outline-none backdrop-blur-sm dark:bg-neutral-950/50"
              align="end" side="bottom" :side-offset="8"
            >
              <DropdownMenuItem
                :class="['relative flex cursor-pointer select-none items-center rounded-md px-3 py-2 leading-none outline-none text-base sm:text-sm transition-colors duration-200 data-[highlighted]:bg-primary-300/20 dark:data-[highlighted]:bg-primary-100/20 data-[highlighted]:text-primary-400 dark:data-[highlighted]:text-primary-200']"
    ...
    <div class="px-1">
      <div class="relative flex items-center group">
        ...
        <input
          ...
          class="w-full pl-9 pr-4 py-2 text-sm rounded-xl outline-none transition-all duration-200 bg-neutral-100 dark:bg-neutral-800 border-2 border-transparent focus:border-primary-400/50 focus:bg-white dark:focus:bg-black"
        >
      </div>
    </div>

    <div v-if="displayModelsFromIndexedDBLoading">Loading display models...</div>

    <div class="flex-1 overflow-x-auto overflow-y-hidden md:flex-none sm:overflow-x-hidden sm:overflow-y-scroll" h-full w-full>
      <div :class="['w-full flex gap-2', isListView ? 'flex-col px-1' : 'md:grid lg:grid-cols-2 md:grid-cols-1']">
        <div
        ...
        >
          <template v-if="isListView">
            <div class="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-neutral-100 dark:bg-neutral-900">
              ...
              <div v-else class="h-full w-full flex items-center justify-center opacity-50"><div i-solar:question-square-bold-duotone /></div>
            </div>
            <div class="min-w-0 flex-1 px-2">
              <div class="truncate text-sm font-bold dark:text-white">{{ model.name }}</div>
            ...
            <DropdownMenuRoot>
              <DropdownMenuTrigger class="p-1 hover:bg-neutral-400/20 rounded-md transition-colors" @click.stop>
                <div i-solar:menu-dots-bold class="text-neutral-400" />
              </DropdownMenuTrigger>
              <DropdownMenuPortal>
                <DropdownMenuContent
                  class="z-10000 max-w-45 rounded-lg bg-neutral-100/80 p-0.5 shadow-md backdrop-blur-sm dark:bg-neutral-950/80"
                  align="end" side="bottom"
                >
                  <DropdownMenuItem
                    class="relative flex cursor-pointer select-none items-center rounded-md px-3 py-2 text-sm outline-none transition-colors duration-200 data-[highlighted]:bg-red-500/10 data-[highlighted]:text-red-500 text-red-400"
            ...
            <div v-if="highlightDisplayModelCard === model.id" class="i-solar:check-circle-bold ml-1 text-lg text-primary-400" />
          </template>

          <template v-else>
            <div absolute left-3 top-4 z-1>
              <DropdownMenuRoot>
                <DropdownMenuTrigger class="h-7 w-7 flex items-center justify-center rounded-lg bg-neutral-900/20 text-white backdrop-blur-sm transition-colors hover:bg-neutral-900/45">
                  <div i-solar:menu-dots-bold />
                </DropdownMenuTrigger>
                <DropdownMenuPortal>
                  <DropdownMenuContent class="z-10000 max-w-45 rounded-lg bg-neutral-900/30 p-0.5 text-white shadow-md backdrop-blur-sm dark:bg-neutral-950/50" align="start" side="bottom" :side-offset="4">
                    <DropdownMenuItem
            ...
            <div class="h-full min-w-80 w-full lg:min-h-60" aspect="12/16" px-1 py-2>
              <img v-if="model.previewImage" :src="model.previewImage" h-full w-full rounded-lg object-cover>
              <div v-else bg="neutral-100 dark:neutral-900" relative h-full w-full flex flex-col items-center justify-center rounded-lg overflow-hidden>
                <div i-solar:question-square-bold-duotone text-4xl opacity-75 />
              </div>
            </div>
            <div w-full flex flex-col p-2>
               <EditableRoot v-slot="{ isEditing }" :default-value="model.name" class="flex gap-2" auto-resize>
                  <EditableArea class="w-[calc(100%-8px-1rem)] dark:text-white">
                    <EditablePreview class="line-clamp-1 overflow-hidden text-ellipsis" />
                    <EditableInput class="outline-none w-full" />
                  </EditableArea>
                  <EditableEditTrigger v-if="!isEditing"><div i-solar:pen-2-line-duotone opacity-50 /></EditableEditTrigger>
                  <EditableSubmitTrigger v-else><div i-solar:check-read-line-duotone opacity-50 /></EditableSubmitTrigger>
                </EditableRoot>
              ...
              <Button class="hidden md:block mt-2" variant="secondary" @click.stop="handlePick(model)">Pick</Button>
            </div>
          </template>
        </div>
      </div>
    ...
    <div v-if="totalPages > 1" class="mt-2 flex items-center justify-center gap-4 border-t border-neutral-400/10 py-2">
      <Button variant="secondary" size="sm" :disabled="currentPage === 1" @click="currentPage--"><div i-solar:alt-arrow-left-linear /></Button>
      <span class="text-xs text-neutral-500 font-mono">{{ currentPage }} / {{ totalPages }}</span>
      <Button variant="secondary" size="sm" :disabled="currentPage === totalPages" @click="currentPage++"><div i-solar:alt-arrow-right-linear /></Button>
    </div>

    <Button class="block md:hidden" @click="handleMobilePick()">Confirm</Button>
  </div>
</template>
