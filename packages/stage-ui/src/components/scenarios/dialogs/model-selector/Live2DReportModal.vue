...
<template>
  ...
  <DialogRoot v-if="isDesktop" :open="showDialog" @update:open="(value) => (showDialog = value)">
    <DialogPortal>
      ...
      />
      <DialogContent
        class="fixed left-1/2 top-1/2 z-[9999] max-h-full max-w-xl w-[92dvw] transform overflow-y-scroll rounded-2xl bg-white p-6 shadow-xl outline-none backdrop-blur-md scrollbar-none -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:bg-neutral-900"
      >
        <div class="mb-4 flex items-center justify-between gap-2">
          <DialogTitle class="text-lg text-neutral-900 font-semibold dark:text-neutral-100">
            Live2D Model Audit Report
          </DialogTitle>
          <Button size="sm" variant="secondary" @click="handleClose">Close</Button>
        </div>
        ...
        <div v-if="report" class="flex flex-col gap-4">
          ...
          <div class="max-h-96 overflow-y-auto pr-2 text-sm space-y-4">
            <div class="grid grid-cols-2 gap-2 rounded bg-neutral-100/50 p-2 dark:bg-neutral-800/50">
              <div>
                Structure:
                <span font-mono>{{ report.structureType }}</span>
              </div>
              <div>
                Files:
                <span font-mono>{{ report.totalFiles }}</span>
              </div>
            ...
            <div v-if="report.errors.length > 0" class="space-y-1">
              ...
              <ul class="list-none pl-0 space-y-1">
                <li
                  ...
                  :key="i"
                  class="flex items-center justify-between gap-2 rounded bg-red-50/50 p-2 text-red-800 dark:bg-red-900/20 dark:text-red-300"
                >
                  <span>{{ err }}</span>
                  <Button
                    v-if="canFixError(err)"
                    size="sm"
                    variant="secondary-muted"
                    class="h-6 px-2 text-[10px] tracking-wider uppercase"
                    @click="handleFix(err)"
                  >
                    Quick Fix
                  </Button>
            ...
            <div v-if="report.warnings.length > 0" class="space-y-1">
              ...
              <ul class="list-none pl-0 space-y-1">
                <li
                  ...
                  :key="i"
                  class="rounded bg-yellow-50/50 p-2 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300"
                >
                  {{ w }}
                </li>
              </ul>
          ...
          <div class="mt-2 flex justify-end gap-2">
            ...
            <div v-else class="flex items-center gap-1 text-xs text-red-500 italic">
              <div i-solar:danger-triangle-bold />
              Invalid models cannot be imported
            </div>
          </div>
        </div>
      </DialogContent>
  ...
  <DrawerRoot v-else :open="showDialog" should-scale-background @update:open="(value) => (showDialog = value)">
    <DrawerPortal>
      ...
      >
        <DrawerHandle />
        <div class="mb-4 flex items-center justify-between gap-2">
          <div class="text-lg text-neutral-900 font-semibold dark:text-neutral-100">Model Audit Report</div>
          <Button size="sm" variant="secondary" @click="handleClose">Close</Button>
        </div>
        ...
        <div v-if="report" class="flex flex-1 flex-col gap-4 overflow-y-auto pb-4">
          ...
          >
            ...
            <div v-else i-solar:close-circle-bold-duotone text-2xl />
            <span>
              {{ report.status }}: {{ report.fileName.slice(0, 20) }}{{ report.fileName.length > 20 ? '...' : '' }}
            </span>
          </div>
