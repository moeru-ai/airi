...
const triageDescription = computed(
  () => props.triageContextSummary || t('settings.dialogs.bug-report.triage-description'),
)
...
function onUploadMediaFromLibraryChange(next: boolean) {
  if (!next) screenshotFiles.value = undefined
}
</script>

<template>
  <div :class="['min-h-0 min-w-0 w-full flex flex-1 flex-col gap-4']">
    <Textarea
      v-model="description"
      :class="['min-h-44 text-sm']"
    ...
    <div v-if="includeTriageContext" :class="['flex flex-col gap-2']">
      <FieldCheckbox
    ...
    <ContainerError v-if="submitError" :error="submitError" height-preset="sm" @feedback="emit('feedback')" />
