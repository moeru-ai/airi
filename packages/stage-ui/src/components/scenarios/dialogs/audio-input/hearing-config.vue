    ...
    <div class="flex flex-col items-center justify-center py-2">
      <div class="relative h-28 w-28 select-none">
        ...
        <div
          ...
          :style="{
            transform: `translate(-50%, -50%) scale(${1 + (volumeLevel / 100) * 0.35})`,
            opacity: String(0.25 + (volumeLevel / 100) * 0.25),
          }"
        ...
        <div
          ...
          :style="{
            transform: `translate(-50%, -50%) scale(${1.2 + (volumeLevel / 100) * 0.55})`,
            opacity: String(0.15 + (volumeLevel / 100) * 0.2),
          }"
        ...
        <div
          ...
          :style="{
            transform: `translate(-50%, -50%) scale(${1.5 + (volumeLevel / 100) * 0.8})`,
            opacity: String(0.08 + (volumeLevel / 100) * 0.15),
          }"
      ...
      <div v-if="!props.granted" class="mt-3 w-full">
        <Callout theme="orange" label="Microphone permission required">
          <div class="text-sm">
            The app doesn't have permission to access your microphone. Please grant microphone access in your system
            settings to enable audio input.
          </div>
        </Callout>
      </div>
    </div>
