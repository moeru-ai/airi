...
>(() => [
  {
    value: 'camera',
    label: t('settings.vrm.scale-and-position.eye-tracking-mode.options.option.camera'),
    class: 'col-start-3',
  },
  {
    value: 'mouse',
    label: t('settings.vrm.scale-and-position.eye-tracking-mode.options.option.mouse'),
    class: 'col-start-4',
  },
  {
    value: 'none',
    label: t('settings.vrm.scale-and-position.eye-tracking-mode.options.option.disabled'),
    class: 'col-start-5',
  },
])
...
<template>
  ...
  >
    <template v-if="allowExtractColors">
      ...
      <Button
        variant="secondary"
        :disabled="controlsLocked || !canExtractColors"
        @click="$emit('extractColorsFromModel')"
      >
    ...
    <div grid="~ cols-5 gap-1" p-2 :class="settingsLockClass">
      ...
      <div class="text-xs">{{ t('settings.vrm.scale-and-position.eye-tracking-mode.title') }}:</div>
      <div />
      ...
      />
      <PropertyColor v-model="directionalLightColor" :disabled="controlsLocked" label="Directional Light Color" />
      ...
      />
      <PropertyColor v-model="ambientLightColor" :disabled="controlsLocked" label="Ambient Light Color" />
    </div>
    <div>
      <div :class="['px-2', 'pt-2', 'text-xs', 'text-neutral-500', 'dark:text-neutral-400']">Environment</div>
      ...
      <div v-if="envSelect === 'hemisphere'">
        ...
        <div grid="~ cols-5 gap-1" p-2 :class="settingsLockClass">
          ...
          />
          <PropertyColor v-model="hemisphereSkyColor" :disabled="controlsLocked" label="Hemisphere Sky Color" />
          <PropertyColor v-model="hemisphereGroundColor" :disabled="controlsLocked" label="Hemisphere Ground Color" />
        </div>
      </div>
      <div v-else>
        <!-- skybox settings -->
