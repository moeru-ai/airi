...
const statusTheme = computed(() => {
  if (props.viewError || (!props.viewSnapshot && props.runtimeSnapshot.phase !== 'no-model')) return 'orange'
...
function cloneSnapshotForDraft(snapshot: StageViewSnapshotPayload) {
  if (snapshot.reason === 'loaded') return cloneStageViewStateForDraft(snapshot.state)
...
function lockCameraPositionRangeFromSnapshot(snapshot: StageViewSnapshotPayload) {
  if (!snapshot.avatarBounds) return

  if (snapshot.reason !== 'loaded' && cameraPositionRangeLocked.value) return
...
function createNumberModel(
  ...
  return computed({
    ...
    set: (value) => {
      if (!Number.isFinite(value) || !draftState.value) return
...
<template>
  <Callout v-if="statusText || viewError" label="Godot Stage (Experimental)" :theme="statusTheme">
    <p v-if="viewError">
      {{ viewError.message }}
    </p>
    <p v-if="statusText">
  ...
  >
    ...
    <div :class="['grid grid-cols-5', 'gap-1 p-2', ...settingsLockClass]">
      <PropertyNumber v-model="cameraPositionX" :config="cameraPositionConfig" label="Camera X" />
      <PropertyNumber v-model="cameraPositionY" :config="cameraPositionConfig" label="Camera Y" />
      <PropertyNumber v-model="cameraPositionZ" :config="cameraPositionConfig" label="Camera Z" />
      <PropertyNumber v-model="cameraYaw" :config="cameraYawConfig" label="Camera Yaw" />
      <PropertyNumber v-model="cameraPitch" :config="cameraPitchConfig" label="Camera Pitch" />
      <PropertyNumber v-model="cameraFov" :config="cameraFovConfig" label="Camera FOV" />
    </div>
  </Container>
</template>
