...
export const sparkCommandGuidanceOptionSchema = z
  .object({
...
export const sparkCommandPersonaSchema = z
  .object({
    traits: z
      .string()
      .describe('Trait name to adjust behavior. For example, "bravery", "cautiousness", "friendliness".'),
    strength: z.enum(['very-high', 'high', 'medium', 'low', 'very-low']),
  })
  .strict()

export const sparkNotifyCommandGuidanceSchema = z
  .object({
    type: z.enum(['proposal', 'instruction', 'memory-recall']),
    persona: z
      .union([z.array(sparkCommandPersonaSchema), z.null()])
      .describe(
...
export const sparkNotifyCommandItemSchema = z
  .object({
    ...
    priority: z
      .union([z.enum(['critical', 'high', 'normal', 'low']), z.null()])
      .describe(
        'Semantic priority of the command, this affects how sub-agents prioritize it (queues, interruption queues, mq, etc.).',
      ),
    intent: z
      .union([z.enum(['plan', 'proposal', 'action', 'pause', 'resume', 'reroute', 'context']), z.null()])
      .describe(
    ...
    guidance: z
      .union([sparkNotifyCommandGuidanceSchema, z.null()])
      .describe(
...
export const sparkCommandMetadataEntrySchema = z
  .object({
...
export const sparkCommandContextSchema = z
  .object({
    ...
    metadata: z
      .union([z.array(sparkCommandMetadataEntrySchema), z.null()])
      .describe('JSON-like metadata for the context update, expressed as key-value pairs for schema compatibility.'),
  })
  .strict()

export const sparkCommandGuidanceSchema = z
  .object({
    type: z.enum(['proposal', 'instruction', 'memory-recall']),
    persona: z
      .union([z.array(sparkCommandPersonaSchema), z.null()])
      .describe('Persona traits that shape the target behavior.'),
    options: z.array(sparkCommandGuidanceOptionSchema).min(1).describe('Concrete execution options for the target.'),
  })
  .strict()

export const sparkCommandToolSchema = z
  .object({
    ...
    interrupt: z
      .union([sparkCommandInterruptSchema, z.null()])
      .describe('Whether the command should preempt current work.'),
    ...
    parentEventId: z
      ...
      .describe('Optional parent event ID when this command is a response to another event.'),
    guidance: z
      .union([sparkCommandGuidanceSchema, z.null()])
      .describe('Structured guidance for how the target should interpret and execute the command.'),
    contexts: z
      .union([z.array(sparkCommandContextSchema), z.null()])
      .describe('Optional context updates to attach to the command.'),
  })
  .strict()
...
export function normalizeSparkCommandPersona(
  ...
  if (!persona?.length) return undefined
...
export function normalizeSparkCommandDestinations(
  ...
  if (destinations == null) return undefined

  if (Array.isArray(destinations) || 'all' in destinations) return destinations
