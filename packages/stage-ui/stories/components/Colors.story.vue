...
<template>
  <Story group="design-system" title="Colors" :layout="{ type: 'single', iframe: false }" responsive-disabled>
    <template #controls>
    ...
    <Variant title="Shades in opacities">
      <div grid="~ cols-12 rows-12" class="transparency-grid" items-center justify-center>
        <template v-for="(shade, c) in shades" :key="`primary-${shade}`">
          <div self-end p-4 text-center font-mono :style="{ gridArea: `1 / ${c + 2} / span 1 / span 1` }">
            {{ shade }}
          </div>
        </template>

        <template v-for="(opacity, r) in opacities" :key="`primary-${opacity}`">
          <div p-4 text-right font-mono :style="{ gridArea: `${r + 2} / 1 / span 1 / span 1` }">/{{ opacity }}</div>

          <div
            v-for="(shade, c) in shades"
            :key="`primary-${shade}/${opacity}`"
            class="cursor-crosshair [&_.color-label]:hover:op-100"
            :class="[`bg-primary-${shade}/${opacity}`]"
            h-82px
            flex
            items-center
            justify-center
