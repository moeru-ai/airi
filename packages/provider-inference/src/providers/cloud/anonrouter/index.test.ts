...
/**
 * Resolved shape of the provider configuration after the schema applies its
 * defaults. The definition keeps its input type private, so the test names only
 * the two fields it reads back.
 */
...
/**
 * Builds the provider the way the settings page does: parse the declared schema
 * so the default base URL is applied, then hand the resolved values to
 * `createProvider`. Reading the endpoint back out of the schema keeps the
 * request assertions tied to the definition instead of to a literal repeated
 * in the test.
 */
