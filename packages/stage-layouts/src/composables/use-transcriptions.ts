...
export function useTranscriptions(options: TranscriptionOptions) {
  ...
  const startStreaming = async () => {
    ...
    if (!hearingConfigured.value) {
      console.info('No transcription provider configured. Auto-configuring Web Speech API as default', {
        source: 'useTranscriptions',
      })
      ...
      if (!isWebSpeechAvailable) {
        ...
        const errorMsg =
          'Web Speech API is not available and no transcription provider is configured. Please go to Settings > Modules > Hearing to configure a transcription provider. '
        console.error(
          errorMsg,
          'Browser support:',
          {
    ...
    if (!supportsStreamInput.value) {
      const errorMsg =
        'Streaming input not supported by the selected transcription provider. Please select a provider that supports streaming (e.g., Web Speech API).'
      ...
      if (!stream.value) {
        ...
        if (!stream.value && hearingEnabled.value) {
          ...
          try {
            ...
            console.error('Timed out waiting for audio stream. Stopping transcription.', {
              source: 'useTranscriptions',
            })
            isListening.value = false
            return
    ...
    if (!stream.value) {
      const errorMsg =
        'Failed to get audio stream for transcription. Please check microphone permissions and ensure a device is selected.'
