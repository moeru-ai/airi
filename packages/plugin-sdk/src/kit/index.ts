...
type KitValuePrimitive = boolean | null | number | string

type IsAny<TValue> = 0 extends (1 & TValue) ? true : false

/**
 * Checks a payload structurally because ordinary interfaces do not extend the
 * string-indexed {@link HostDataValue} record type in TypeScript.
 */
type IsKitValueCompatible<TValue> = IsAny<TValue> extends true
  ? false
  : [TValue] extends [KitValue]
      ? true
      : TValue extends KitValuePrimitive
        ? true
        : TValue extends (...args: never[]) => unknown
          ? false
          : TValue extends unknown[]
            ? false extends IsKitValueCompatible<TValue[number]> ? false : true
            : TValue extends object
              ? keyof TValue extends never
                ? false
                : false extends {
                  [TKey in keyof TValue]-?: IsKitValueCompatible<TValue[TKey]>
                }[keyof TValue] ? false : true
              : false

type KitMethodValidation<TInput, TOutput> = [TInput] extends [undefined]
  ? IsKitValueCompatible<TOutput> extends true
    ? []
    : [error: 'Kit method output must be a KitValue']
  : IsKitValueCompatible<TInput> extends true
    ? IsKitValueCompatible<TOutput> extends true
      ? []
      : [error: 'Kit method output must be a KitValue']
    : [error: 'Kit method input must be a KitValue or undefined']

type KitEventValidation<TPayload> = IsKitValueCompatible<TPayload> extends true
  ? []
 ...
 *
 * The input uses `undefined` only when the method accepts no argument. Every
 * input and output value must be compatible with the Host-safe KitValue model.
 */
export function defineKitMethod<
  TInput,
  TOutput,
>(...validation: KitMethodValidation<TInput, TOutput>): KitMethodContract<TInput, TOutput> {
  void validation
  return { kind: 'method' }
}

/** Defines one typed event with a Host-safe KitValue payload. */
export function defineKitEvent<TPayload>(...validation: KitEventValidation<TPayload>): KitEventContract<TPayload> {
  void validation
  return { kind: 'event' }
}
