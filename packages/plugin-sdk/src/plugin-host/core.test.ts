...
describe('for ExtensionHost', () => {
  ...
  it('revokes a generated Kit client when its Consumer stops', async () => {
    const host = new ExtensionHost()
    const kit = createActivityKit()
    ...
    let methodCalls = 0
    const provider = defineExtension({
      id: 'consumer-revocation-provider',
      setup(ctx) {
        ctx.kits.provide(kit, {
          methods: {
    ...
    let client: KitClientOf<typeof kit> | undefined
    const consumer = defineExtension({
      id: 'consumer-revocation-consumer',
      async setup(ctx) {
    ...
    const pendingCall = retainedClient.getCurrentActivity()
    ...
    // ROOT CAUSE:
    //
    // Generated methods checked only whether the Provider was current. A
    // retained client could call the Provider after its Consumer stopped, and
    // an in-flight call could still return into stopped Consumer code. The
    // client lifecycle now rejects both paths.
    await expect(pendingCall).rejects.toThrow('Consumer stopped before method `getCurrentActivity` completed')
    await expect(retainedClient.getCurrentActivity()).rejects.toThrow('Consumer is not available')
  ...
  it('discards Kit availability that becomes stale during resolution', async () => {
    const host = new ExtensionHost()
    let replaced = false
    const replacementKit = defineKit({
      id: 'kit.resolution-race',
      version: '2.0.0',
      createClient: () => ({ version: 2 }),
    })
    const kit = defineKit({
      id: replacementKit.id,
      version: '1.0.0',
      createClient: () => {
        if (!replaced) {
          replaced = true
          void host.unregisterKitApi(kit.id)
          host.registerKitApi(replacementKit)
        }
        return { version: 1 }
      },
    })
    ...
    const consumer = defineExtension({
      id: 'resolution-race-watcher-consumer',
      setup(ctx) {
        ctx.kits.watch(kit, (availability) => {
          if (!availability.available) {
            observedReasons.push(availability.reason)
          }
        })
      },
    })
    ...
    // ROOT CAUSE:
    //
    // Delivery id was checked only before resolution. Replacing the Kit in
    // createClient queued a delivery, but the old result reached the callback
    // with a stale missing-kit snapshot. The Host now checks again after
    // resolution and drops that snapshot before the Consumer callback.
    await vi.waitFor(() => expect(observedReasons).toEqual(['incompatible-version']))
  })
  ...
  it('normalizes exceptions thrown while validating Provider output', async () => {
    const host = new ExtensionHost()
    const kit = defineKitContract({
      id: 'dev.airi.throwing-output',
      version: '1.0.0',
      methods: {
        read: defineKitMethod<undefined, { value: string }>(),
      },
    ...
    const rawFailure = { capability: () => 'Provider capability' }
    const output = new Proxy({ value: 'safe' }, {
    ...
    const provider = defineExtension({
      id: 'throwing-output-provider',
      setup(ctx) {
        Reflect.apply(ctx.kits.provide, ctx.kits, [kit, {
          methods: { read: () => output },
        }])
      },
    })
    ...
    // ROOT CAUSE:
    //
    // Kit output validation ran outside the Provider-call error boundary. A
    // Proxy getter could therefore throw an arbitrary Provider-owned value
    // directly into Consumer code. The KitValue boundary now replaces
    // validation and copy exceptions with a Host-owned TypeError.
    expect(observedError).toBeInstanceOf(Error)
    expect(observedError).not.toBe(rawFailure)
    expect(observedError).toMatchObject({
      message: 'Kit method `dev.airi.throwing-output.read` output could not be validated as a KitValue.',
    })
  })
