...
interface ExtensionKitClientLifecycle {
  revoked: boolean
}
...
function cloneKitValue(value: unknown, source: string): KitValue {
  const result = (() => {
    try {
      return safeParse(hostDataValueSchema, value)
    }
    catch {
      throw new TypeError(`${source} could not be validated as a KitValue.`)
    }
  })()
  if (!result.success) {
  ...
  try {
    ...
    throw new TypeError(`${source} could not be copied as a KitValue.`)
  }
}
...
export class ExtensionHost {
  ...
  private createExtensionKitClient<TContract extends KitContract>(
    ...
    session: ExtensionSession,
    subscriptions: DisposableStore,
    lifecycle: ExtensionKitClientLifecycle,
    moduleId?: string,
    ...
    const isConsumerAvailable = () => !lifecycle.revoked
      && (session.phase === 'setting-up' || session.phase === 'ready')

    for (const methodName of Object.keys(contract.methods)) {
      client[methodName] = async (input?: unknown) => {
        if (!isConsumerAvailable()) {
          throw new Error(`Kit \`${contract.id}\` Consumer is not available.`)
        }
        ...
        try {
          ...
          if (!isConsumerAvailable()) {
            throw new Error(`Kit \`${contract.id}\` Consumer stopped before method \`${methodName}\` completed.`)
          }
          throw new Error(
  ...
  private async resolveKitApi<TKit extends ConsumableKit>(
    ...
    const clientLifecycle: ExtensionKitClientLifecycle = { revoked: false }
