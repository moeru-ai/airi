...
const defaultPromptByTokenSource: Record<typeof discordTokenSource, string> = {
  auto: 'Configure AIRI Discord automatically. Do not ask the human for the token. Prefer retrieving the token from the live Discord browser or Developer Portal session by using browser_dom_* tools, browser_cdp_collect_elements when rerouted, clipboard_read_text, clipboard_write_text, and desktop/browser control. If no live token can be retrieved, fall back to local sources such as .env. Then open AIRI Discord settings, enable the module, fill the token, save it, and verify the module is configured. Keep going until verified, not narrated.',
  portal: 'Configure AIRI Discord automatically. Do not ask the human for the token and do not use .env as the primary source. Use browser_dom_* tools, browser_cdp_collect_elements when rerouted, clipboard_read_text, clipboard_write_text, and desktop/browser control to retrieve the token from the live Discord browser or Developer Portal session, then open AIRI Discord settings, enable the module, fill the token, save it, and verify the module is configured. Keep going until verified, not narrated.',
