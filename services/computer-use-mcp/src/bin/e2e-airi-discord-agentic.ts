...
const discordTokenSource = (['auto', 'local', 'portal'].includes((env.AIRI_E2E_DISCORD_TOKEN_SOURCE || '').trim().toLowerCase())
  ? (env.AIRI_E2E_DISCORD_TOKEN_SOURCE || '').trim().toLowerCase()
  : 'auto') as 'auto' | 'local' | 'portal'
...
async function main() {
  ...
  try {
    ...
    addTimeline('terminated-stale-processes', { discordBot: verifyDiscordBot, stage: true })
