/**
 * Benchmark Harness v1: bounded agentic coding failure clusters.
 *
 * Coverage goals (Day5):
 * - TS repo baseline_noise boundary
 * - TS repo new_red boundary
 * - wrong_target diagnosis cluster
 * - missed_dependency diagnosis cluster
 * - Python unsupported/fallback branch
 * - dirty-tree baseline_noise stability
 *
 * Output:
 * - Structured JSON report with caseId + traceId (+ rootCause/baselineComparison)
 */
...
function setupGitRepo(dir: string) {
  ...
  execFileSync('git', ['config', 'user.email', 'benchmark-coding-agentic-v1@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'AIRI Benchmark Harness'], { cwd: dir })
...
function createTsFixture(prefix: string, options: { dirtyTree?: boolean } = {}) {
  const dir = mkdtempSync(join(tmpdir(), `benchmark-coding-agentic-v1-ts-${prefix}-`))
...
function createPythonFixture(prefix: string) {
  const dir = mkdtempSync(join(tmpdir(), `benchmark-coding-agentic-v1-py-${prefix}-`))
...
async function createClient() {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      COMPUTER_USE_SESSION_TAG: 'benchmark-coding-agentic-v1',
  ...
  const client = new Client({
    name: '@proj-airi/benchmark-coding-agentic-v1',
    version: '0.1.0',
  })
...
function summarize(records: BenchmarkRecord[]) {
...
async function main() {
  console.info('╔═══════════════════════════════════════════════════════════════════════╗')
  console.info('║  Benchmark Harness v1: coding agentic failure clusters              ║')
  console.info('╚═══════════════════════════════════════════════════════════════════════╝')
  ...
  try {
    ...
    const outputPath = env.COMPUTER_USE_BENCHMARK_OUTPUT?.trim()
      || join(packageDir, 'artifacts', 'coding-agentic-benchmark-v1.json')

    mkdirSync(dirname(outputPath), { recursive: true })
    writeFileSync(outputPath, JSON.stringify({
      generatedAt: new Date().toISOString(),
      harness: 'benchmark-coding-agentic-v1',
      summary,
      records,
    }, null, 2), 'utf8')

    for (const record of records) {
      console.info(`  ${record.pass ? '✓' : '✗'} ${record.caseId} :: cluster=${record.cluster} :: traceId=${record.traceId || 'n/a'} :: rootCause=${record.rootCauseType || 'n/a'}`)
    }
    ...
    console.info('\n✅ Benchmark passed: all Day5 v1 scenarios succeeded.')
  }
  finally {
