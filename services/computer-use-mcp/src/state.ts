...
export interface CodingRunState {
  ...
  /**
   * Operational memory seeds derived after the last verification gate decision.
   * Replaced wholesale each cycle — no historical accumulation.
   * Used by the next workflow invocation to apply lightweight bias.
   */
  operationalMemorySeeds?: import('./coding/coding-memory-taxonomy').CodingOperationalMemorySeed[]
  /**
   * Short human-readable summary of the current operational memory seeds.
   * Generated by `summarizeOperationalMemory`; suitable for embedding in logs.
   */
  lastOperationalMemorySummary?: string
}
