...
### Processing Status
```bash
# Check embedding regeneration status
curl -H "Authorization: Bearer your_api_key" \
  http://localhost:3001/api/settings/regeneration-status
```
...
-- Check memory fragments by type
SELECT memory_type, COUNT(*) as count
FROM memory_fragments
WHERE deleted_at IS NULL
GROUP BY memory_type;

-- Check recent memory access patterns
...
# Setup database (generate schema and run migrations)
pnpm db:setup
