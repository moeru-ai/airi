...
export function extractJavaScriptCandidate(input: string): string {
  ...
  if (normalized.startsWith('```')) {
    ...
    if (lastFenceIndex === normalized.length - 3) {
      const firstNewlineIndex = normalized.indexOf('\n')
      if (firstNewlineIndex !== -1) {
        const info = normalized.slice(3, firstNewlineIndex).trim().toLowerCase()
        const allowed = new Set(['', 'js', 'javascript', 'ts', 'typescript'])
        if (allowed.has(info)) {
          const body = normalized.slice(firstNewlineIndex + 1, lastFenceIndex)
          return body.trim()
        }
      }
    }
