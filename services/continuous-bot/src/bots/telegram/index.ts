...
async function handleLoop(state: BotSelf, msgs?: LLMMessage[]) {
  if (msgs == null) {
    msgs = messages(
      ...
      system(''
        + '1: {"action":"listChats"}: List all available chats, best to do before you want to send a message to a chat.\n'
        + '2: {"action":"sendMessage","content":"<content>","groupId":"id of chat to send to"}: Send a message to a specific chat group. If you want to express anything to anyone or your friends in group, you can use this action.\n'
        + '3: {"action":"readMessages","groupId":"id of chat to send to"}: Read unread messages from a specific chat group. If you want to read the unread messages from a specific chat group, you can use this action.\n'
        + '4. {"action":"continue"}: Continue the current task, which means to keep your current state unchanged, I\'ll ask you again in next tick.\n'
        + '5. {"action":"break"}: Take a break, which means to clear out ongoing tasks, but keep the short-term memory, and I\'ll ask you again in next tick.\n'
        + '6. {"action":"sleep"}: Sleep, which means to clear out ongoing tasks, and clear out the working memory, and I\'ll ask you again in next tick.\n'
        // + '7. {"action":"lookupShortTermMemory","query":"","category:"chat or self"}: Look up the short-term, which means to recall the short-term memory from memory component.\n'
        // + '8. {"action":"lookupLongTermMemory","query":"","category": "chat or self"}: Look up the long-term, which means to recall the long-term memory from memory component.\n'
        // + '9. {"action":"memorizeShortMemory","content":"<content>","tags":["keyword tag"]}: Memorize to short-term memory, which means to append things the short-term memory which will be included for a while, but will be eventually forgot.\n'
        // + '10. {"action":"memorizeLongMemory","content":"<content>","tags":["keyword tag"]}: Memorize to long-term memory, which means to append things the long-term memory which will be included for a long time, and hard to forget.\n'
        // + '11. {"action":"forgetShortTermMemory","where":{"id":"<id of memory>"}}: Remove specific short-term memory entry from the memory component.\n'
        // + '12. {"action":"forgetLongTermMemory","where":{"id":"<id of memory>"}}: Remove specific long-term memory entry from the memory component.\n'
        // + '13: {"action":"searchGoogle","query":"<query>"}: Search Google with the query.\n'
        + '',
      ),
      system(''
...
export async function startTelegramBot() {
  ...
  bot.api.config.use(hydrateFiles(bot.api.token))
