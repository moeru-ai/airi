  ...
  try {
    ...
    await Promise.all(photoBase64s.map(async (base64, index) => {
      const res = await generateText({
        ...
        messages: messages(
          system(`This is a photo sent by user ${message.from.first_name} ${message.from.last_name} on Telegram, with the caption ${message.caption} Please describe what do you see in this sticker.`),
