...
import { deserializeWorkerMessage, serializeWorkerMessage } from './protocol.js'
    ...
    stdout.on('data', (chunk: Buffer) => {
      ...
      while (true) {
        if (this.stdoutExpectedLength === null) {
          if (this.stdoutBuffer.length < HEADER_SIZE)
            break
        ...
        if (this.stdoutBuffer.length < this.stdoutExpectedLength)
          break
    ...
    stdin.on('data', (chunk: Buffer) => {
      ...
      while (true) {
        if (this.stdinExpectedLength === null) {
          if (this.stdinBuffer.length < HEADER_SIZE)
            break
        ...
        if (this.stdinBuffer.length < this.stdinExpectedLength)
          break
