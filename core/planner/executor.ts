...
import { createCancellationToken, withTimeout } from '../tasks/cancellation.js'
...
export class PlanExecutor {
  ...
  cancelPlan(planId: PlanId, reason?: string): Plan | undefined {
    ...
    if (!plan || (plan.id as string) !== planId)
      return undefined
  ...
  async persistState(): Promise<void> {
    if (!this.options.eventStore || !this.runningPlan)
      return
  ...
  private async executeStepsInOrder(plan: Plan): Promise<void> {
    ...
    while (pending.size > 0 || running.size > 0) {
      ...
      for (const stepId of pending) {
        if (slotsUsed >= this.options.concurrency)
          break
        const deps = dependencyIds.get(stepId) ?? []
