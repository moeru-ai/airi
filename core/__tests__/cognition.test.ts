...
function createTestProposedStep(overrides: Partial<ProposedStep> & { id: string; name: string; action: string }): ProposedStep {
	return {
		input: {},
		...overrides,
	}
...
describe("proposalToPlan", () => {
	...
	it("replaces temporary step IDs with proper StepIds", () => {
		const steps = [
			createTestProposedStep({ id: "temp-1", name: "Step 1", action: "read_file" }),
		]
		const proposal = createProposal(createReasoningId("r1"), "Test", steps)
...
describe("extractCapabilityRequirements", () => {
	...
	it("returns empty array when no capabilities", () => {
		const steps = [
			createTestProposedStep({ id: "s1", name: "S1", action: "read_file" }),
		]
...
describe("PlanValidator", () => {
	...
	it("normalizes proposal with default timeouts", () => {
		...
		const normalized = validator.normalize(proposal)
...
describe("MockCognitionProvider", () => {
	...
	it("throws when no fixture matches and no default", async () => {
		await expect(
			provider.generatePlanProposal(makeRequest("unknown prompt")),
		).rejects.toThrow("No matching fixture or default proposal")
	})
	...
	it("throws on cancellation", async () => {
		...
		await expect(
			provider.generatePlanProposal(makeRequest("scan"), source.token),
...
describe("CognitionCoordinator", () => {
	...
	it("accepts a validated proposal", async () => {
		...
		const plan = coordinator.acceptProposal(proposal, { sessionId: "test-session" })
