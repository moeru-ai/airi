...
describe("bootstrap()", () => {
	...
	it("activates the code module", async () => {
		const core = await bootstrap()

		expect(core.registry.isActive("code")).toBe(true)
		expect(core.registry.get("code")).toBeDefined()
	})

	it("activation result shows 1 succeeded", async () => {
		const core = await bootstrap()

		expect(core.activationResult!.total).toBe(1)
		expect(core.activationResult!.succeeded).toBe(1)
		expect(core.activationResult!.failed).toBe(0)
	})
	...
	it("emits module.activated event during bootstrap", async () => {
		// We need to listen before bootstrap activates modules.
		// Since bootstrap creates its own EventBus internally,
		// we test this indirectly through the activation result.
		const core = await bootstrap()

		const activated = core.activationResult!.results.find(
			(r) => r.id === "code" && r.state === "active",
		)
