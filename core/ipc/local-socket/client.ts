...
export class LocalSocketClientTransport implements IpcClientTransport {
	...
	private heartbeatTimer: ReturnType<typeof ReturnType<typeof setInterval>> | null = null
	...
	private doConnect(): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			...
			const socket = netConnect(
				isTcp ? { port: parseInt(this.socketPath.split(":")[1]), host: this.socketPath.split(":")[0] } : this.socketPath,
			)
	...
	private scheduleReconnect(): void {
		...
		console.log(
			`[LocalSocketClient] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})...`,
		)
