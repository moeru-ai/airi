...
function parseJsTsImports(content: string): string[] {
  const imports = [
    ...collectMatches(content, /import\s+(?:.+(?:[\n\r\u2028\u2029]\s*|[\t\v\f \xA0\u1680\u2000-\u200A\u202F\u205F\u3000\uFEFF])from\s+)?['"]([^'"]+)['"]/g),
...
function determineNodeType(
  ...
  if (parts[0] === 'test' || parts[0] === 'tests')
    return 'test'
  if (parts[0] === 'config' || parts[0] === 'configs')
    return 'config'
  if (parts[0] === 'utils' || parts[0] === 'helpers' || parts[0] === 'lib')
    return 'utility'
  if (parts[0] === 'services')
    return 'service'
  if (parts[0] === 'components' || parts[0] === 'widgets')
    return 'component'
  if (parts[0] === 'modules' || parts[0] === 'features')
    return 'module'
