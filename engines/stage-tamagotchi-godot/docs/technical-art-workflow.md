...
Every renderer-facing change needs three artifacts before it can be accepted:
...
2. Baseline evidence.
   Capture the current AIRI output before changing the renderer. If the change
   is a refactor, this is the behavior that must remain visually stable.
3. Comparison evidence.
   Capture the same scene after the change and compare it against the accepted
   baseline. Do not rely on memory or a manually inspected live viewport alone.

The current automated baseline command is:

```powershell
pnpm -F @proj-airi/stage-tamagotchi-godot verify:visual-baseline
```

To accept a new local baseline after manual review:

```powershell
pnpm -F @proj-airi/stage-tamagotchi-godot verify:visual-baseline -- --update-baseline
```

When validating that the edge-light feature did not change the feature-off
baseline:

```powershell
pnpm -F @proj-airi/stage-tamagotchi-godot verify:visual-baseline:edge-off
```

The baseline image lives at:

```text
engines/stage-tamagotchi-godot/tests/visual-baselines/avatarSampleA-main-stage.png
```

The latest comparison capture lives under:

```text
engines/stage-tamagotchi-godot/artifacts/visual-baseline/
```

`artifacts/` is ignored because it is a run output, not source.

The automated baseline capture is end-to-end: it launches the real main stage,
loads the tracked avatar through the WebSocket host path, and captures the
visible Godot window client area. Internal viewport texture readback is not
acceptable as renderer evidence because it can bypass the final display
conversion and produce darker, higher-contrast PNGs than the actual window.

For compositor features such as Xiaoer-style edge light, also dump pipeline
stages from the same final-window path:
...
3. Freeze the baseline.
   Run `verify:visual-baseline -- --update-baseline`, inspect the PNG, and only
   then treat it as the local reference for the next change.
...
5. Compare before judging.
   Run `verify:visual-baseline`. If the command fails, inspect the current PNG
   and the metrics before changing code again.

6. Classify the difference.
   Put the difference into one bucket before fixing:
   - expected visual change
   - baseline drift caused by the intended change
   - regression in an unrelated visual property
   - capture instability
   - fixture or camera mismatch

7. Record the acceptance.
   If the image changes intentionally, update the baseline and write why. If the
   code change is supposed to preserve behavior, the baseline comparison must
   pass without updating the baseline.
...
- Use final window output for visual acceptance. `GetViewport().GetTexture()`
  ...
  baseline for color, contrast, glow, rim light, or tone-mapping judgments.
...
1. Current baseline capture exists and has been manually accepted.
2. `verify:visual-baseline` passes on the refactor-only pipeline.
...
4. The planned edge-light implementation is behind a toggle or stage boundary so
   feature-off output can still compare against the baseline.
...
and a new visual effect at the same time without proving baseline parity.
