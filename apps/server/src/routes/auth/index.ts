...
export async function createAuthRoutes(deps: AuthRoutesDeps) {
  const trustedClientIds = [
    deps.env.OIDC_CLIENT_ID_WEB,
    deps.env.OIDC_CLIENT_ID_ELECTRON,
    deps.env.OIDC_CLIENT_ID_POCKET,
  ...
  return new Hono<HonoEnv>()
     ...
     */
    .route('/api/auth/oidc/session', createOIDCSessionRoute(deps.auth, deps.db, trustedClientIds))
    /**
