...
transaction log / audit / llm_request_log 的写入均由 **worker role** 中的 billing event consumer 异步完成（实现：`billing-consumer-handler.ts`）。
...
- **worker role / billing event consumer** — 消费 Redis Stream `billing-events`，将 transaction log、LLM 请求日志异步写入 DB（Redis consumer group 名称仍为 `billing-consumer`，是不可改的 production identifier；但 Railway role 名是 `worker`）
...
- `api` — HTTP 服务
- `worker` — 所有非 API 后台任务的统一 role；billing 域内当前跑：消费 Redis Stream `billing-events` 异步写入 transaction log / LLM 请求日志，以及 admin flux grant batch 轮询
...
- **`debitFlux()`** — 扣费（LLM 请求），事务内：锁行 → 检余额(402) → 更新余额；事务提交后 XADD `flux.debited` 到 Redis Stream，transaction 由 `worker` role 中的 billing event consumer 异步写入
- **`creditFlux()`** — 通用充值
...
| 2. Redis Streams 异步写入 | ✅ 已完成 | debitFlux 事务后 XADD，worker role 中的 billing event consumer 异步写 transaction/请求日志 |
| 3. Stripe 幂等 | ✅ 已完成 | checkout + invoice 事务内幂等检查 |
| 4. LLM 计费优化 | ⚠️ 部分 | 已有 `requestId` 和 DB 事务扣费，待加 tiktoken fallback |
| 5. 部署拆分 | ✅ 已完成 | `bin/run.ts` 两角色启动（api / worker） |
...
- `outbox_events` 表及 outbox-dispatcher 进程 — 已移除，统一由 worker role 处理异步写入
- `cache-sync-consumer` 进程角色 — 已合并进 worker role
