...
Hono-based Node.js resource API. The sibling `apps/auth-server` workspace app owns Better Auth, OIDC, sessions, and account lifecycle; this package owns billing, chat sync, LLM gateway forwarding, and business observability. **Multi-instance deployed on Railway** — design all features assuming N>1 instances sharing the same Postgres and Redis.

## Deployment Model

- Hosted on **Railway**, multiple instances behind a load balancer.
- Independent applications: `apps/server/src/main.ts` and `apps/auth-server/src/main.ts`. There is no runtime role CLI and neither app imports the other service graph.
...
Hono, Drizzle ORM, PostgreSQL, Redis, Stripe, OpenTelemetry, Valibot, injeca (DI), tsx. Better Auth lives only in `apps/auth-server`.
...
pnpm -F @proj-airi/auth-server auth:generate # better-auth → packages/auth-shared/src/schema.ts
```
...
**Entry & DI**: `apps/server/src/main.ts` → `src/server.ts` → `src/app.ts`; `apps/auth-server/src/main.ts` → `src/server.ts` → `src/app.ts`. The workspace apps have separate package manifests, env schemas, Dockerfiles, and composition roots.
...
- **Auth boundary**: `apps/auth-server` owns Better Auth + OIDC. The API's `sessionMiddleware` validates Auth-issued JWTs and fills context but doesn't block; `authGuard` returns 401.
