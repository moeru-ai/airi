-- Idempotent repair for migration 0017's objects.
--
-- A suspected multi-replica boot race could create these tables without
-- recording migration 0017 in drizzle.__drizzle_migrations, leaving orphaned
-- objects that make a later replay of 0017 fail. This migration re-applies
-- the same objects with IF NOT EXISTS / DROP-then-ADD guards so it no-ops on
-- environments where 0017 already applied cleanly, and heals ones where it
-- didn't.
--
-- Do not use dollar-quoted DO blocks here: some migrators split SQL on
