...
var C = class extends AudioWorkletNode {
	...
	volume = 0;
	weights = {};
	constructor(A, I, C = g.wasmModule) {
		super(A, "wlipsync-processor", { processorOptions: {
			wasmModule: C,
			profile: I
		} });
		for (const A of I.mfccs) {
			this.weights[A.name] = 0;
			this.weightVelocities[A.name] = 0;
		}
		this.port.onmessage = this.onMessage.bind(this);
	}
	onMessage(A) {
		const g = A.data.timestamp - this.lastTimestamp;
		this.lastTimestamp = A.data.timestamp;
		const C = Number.isNaN(A.data.volume) ? 0 : A.data.volume;
		let B = Math.log10(C);
		B = (B - this.minVolume) / (this.maxVolume - this.minVolume);
		B = Math.max(Math.min(B, 1), 0);
		[this.volume, this.openCloseVelocity] = I(this.volume, B, this.openCloseVelocity, this.smoothness, g);
		for (const C in this.weights) {
			const B = C === A.data.name ? 1 : 0;
			let Q = this.weightVelocities[C];
			[this.weights[C], Q] = I(this.weights[C], B, Q, this.smoothness, g);
			this.weightVelocities[C] = Q;
		}
	}
	get blockSize() {
		return this.parameters.get("blockSize").value;
	}
	set blockSize(A) {
		this.parameters.get("blockSize").setValueAtTime(A, this.context.currentTime);
	}
};
...
function Q(A) {
	...
	let g = 0;
	const C = () => {
		const A = I.getFloat32(g);
		g += 4;
		return A;
	};
	const Q = () => I.getUint8(g++);
	const E = () => {
		const A = I.getInt32(g);
		g += 4;
		return A;
	};
	const i = () => String.fromCharCode(I.getUint8(g++));
	const D = () => {
		const I = Q();
		const C = B.decode(A.slice(g, g + I));
		g += I;
		return C;
	};
	const o = (A) => [...new Array(A)].map(((A) => C()));
	if (i() !== "W" || i() !== "L" || i() !== "I" || i() !== "P") throw new Error("Invalid file signature for binary wLipSync profile");
	const s = {
		targetSampleRate: E(),
		sampleCount: E(),
		melFilterBankChannels: Q(),
		compareMethod: Q(),
		mfccNum: 12,
		mfccs: [...new Array(Q())],
		mfccDataCount: Q(),
		useStandardization: Q() === 1
	};
	for (let A = 0; A < s.mfccs.length; A++) s.mfccs[A] = {
		name: "",
