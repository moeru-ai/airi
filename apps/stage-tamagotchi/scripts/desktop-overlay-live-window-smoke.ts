...
import { CdpClient, findAvailablePort, findDebugTarget, isRecord, sleep, waitFor, waitForRemoteDebug } from './lib/remote-debug'
...
async function findOverlayTarget(debugPort: number) {
  return await findDebugTarget(
    debugPort,
    'desktop overlay debug target',
    target => target.type === 'page' && target.url.includes('#/desktop-overlay'),
  )
}
