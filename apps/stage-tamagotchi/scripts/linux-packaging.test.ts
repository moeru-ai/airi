...
describe('linux package configuration', () => {
  ...
  it('generates architecture-aware artifact names for Linux ARM64', async () => {
    ...
    expect(artifacts).toEqual(expect.arrayContaining([
      expect.objectContaining({
        extension: 'deb',
        outputFilename: `AIRI-${packageJSON.version}-linux-arm64.deb`,
      }),
      expect.objectContaining({
        extension: 'rpm',
        outputFilename: `AIRI-${packageJSON.version}-linux-aarch64.rpm`,
      }),
      expect.objectContaining({
        extension: 'flatpak',
        outputFilename: `AIRI-${packageJSON.version}-linux-arm64.flatpak`,
      }),
    ]))
  })
  ...
  it('generates architecture-aware artifact names for Linux x64', async () => {
    ...
    expect(artifacts).toEqual(expect.arrayContaining([
      expect.objectContaining({
        extension: 'deb',
        outputFilename: `AIRI-${packageJSON.version}-linux-amd64.deb`,
      }),
      expect.objectContaining({
        extension: 'rpm',
        outputFilename: `AIRI-${packageJSON.version}-linux-x86_64.rpm`,
      }),
      expect.objectContaining({
        extension: 'flatpak',
        outputFilename: `AIRI-${packageJSON.version}-linux-x64.flatpak`,
      }),
    ]))
  })
})
