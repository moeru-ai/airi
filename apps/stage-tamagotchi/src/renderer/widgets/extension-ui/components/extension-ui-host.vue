...
const model = computed<PluginModuleWidgetPayload & Record<string, unknown>>(() =>
  isPlainObject(props.modelValue)
    ? (props.modelValue as PluginModuleWidgetPayload & Record<string, unknown>)
    : ({} as PluginModuleWidgetPayload & Record<string, unknown>),
)
...
const resolvedWidgetProps = computed(() =>
...
const {
  ...
  resolvedIframeSrc,
  iframeMountError,
} = useExtensionUIForModule({
  moduleId,
  inspectPluginHost: () => inspectPluginHost(),
  getPluginAssetBaseUrl: () => getPluginAssetBaseUrl(),
})
...
const {
  ...
  onIframeError,
  onIframeLoad,
} = useIframeMessagePort(iframeElement, {
  moduleId,
...
<template>
  <div :class="['h-full', 'w-full']">
    ...
    >
      ...
      <div v-if="loading" :class="['text-sm', 'opacity-80']">Loading extension UI...</div>
      ...
      <dl
        v-if="moduleSnapshot"
        :class="['grid', 'grid-cols-[auto_1fr]', 'gap-x-3', 'gap-y-2', 'text-xs', 'opacity-80']"
      >
        <dt>Kit</dt>
      ...
      <div :class="['min-h-0', 'flex-1', 'overflow-auto', 'rounded-lg', 'bg-black/15', 'p-3']">
        <pre :class="['whitespace-pre-wrap', 'break-words', 'text-[11px]', 'opacity-80']">{{
          JSON.stringify(
            {
              config: moduleConfig,
