  ...
  <div class="space-y-6">
    <div class="rounded bg-neutral-100 p-3 text-xs font-mono space-y-2 dark:bg-neutral-800">
      ...
      <div class="border-t border-neutral-300 pt-2 dark:border-neutral-700">
        <div>{{ windowRelativeMouse.x.value }} = {{ cursorX }} - {{ windowBounds.x.value }}</div>
        <div>{{ windowRelativeMouse.y.value }} = {{ cursorY }} - {{ windowBounds.y.value }}</div>
      </div>
    </div>

    <!-- Visual Representation -->
    <div
      class="relative border-2 border-neutral-300 rounded bg-white p-8 dark:border-neutral-700 dark:bg-neutral-950"
      style="height: 400px"
    >
      ...
      <div class="absolute inset-8 border-2 border-primary-500 rounded bg-primary-50/20 dark:bg-primary-950/20">
        ...
        <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          ...
          <div
            class="absolute left-1/2 top-1/2 h-0.5 w-8 bg-neutral-300 -translate-x-1/2 -translate-y-1/2 dark:bg-neutral-700"
          />
        ...
        <!-- Corner labels -->
        <div class="absolute text-[10px] text-neutral-500 -left-5 -top-5">(0, 0)</div>
        <div class="absolute text-[10px] text-neutral-500 -right-5 -top-5">({{ windowBounds.width.value }}, 0)</div>
        <div class="absolute text-[10px] text-neutral-500 -bottom-5 -left-5">(0, {{ windowBounds.height.value }})</div>
