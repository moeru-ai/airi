...
async function refetchSources() {
  try {
    ...
    sources.value = nextSources.map((source) => ({
      ...
      appIconURL:
        source.appIcon && source.appIcon.length > 0 ? createObjectUrlFromBytes(source.appIcon, 'image/png') : undefined,
      thumbnailURL:
...
<template>
  <WithScreenCapture :sources-options="sourcesOptions">
    <template #default="{ hasPermissions, requestPermission }">
      <div
        v-if="hasPermissions"
        :class="['flex', 'w-full', 'flex-col', 'items-start', 'gap-4', 'text-neutral-500', 'dark:text-neutral-400']"
      >
        <div
        ...
        >
          ...
          <div :class="['flex', 'w-full', 'items-center', 'gap-3', 'overflow-x-auto']">
            ...
            >
              ...
              <video autoplay muted playsinline :srcObject="stream" class="h-140px w-auto" />
            </div>
          </div>
        </div>
        ...
        <div :class="['flex', 'w-full', 'flex-col', 'gap-3', 'pb-6']">
          ...
          <div v-else :class="['grid', 'gap-3', 'grid-cols-1', 'md:grid-cols-2', 'xl:grid-cols-4', '3xl:grid-cols-5']">
            <div
            ...
            >
              <div
                :class="[
                  ...
                  'group-hover:outline-neutral-100',
                  'dark:group-hover:outline-neutral-800',
                  'group-focus-within:outline-2',
                  'group-focus-within:outline-neutral-100',
                  'dark:group-focus-within:outline-neutral-800',
                ]"
              >
                <div
                  :class="[
                ...
                />
                <div v-else class="i-solar:forbidden-circle-line-duotone absolute inset-0 m-auto h-10 w-10 bg-light" />
              </div>
