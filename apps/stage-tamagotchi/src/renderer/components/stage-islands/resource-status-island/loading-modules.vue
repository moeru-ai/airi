...
<template>
  <div flex flex-col gap-3>
          ...
          <WindowRouterLink :to="`/settings/modules/${moduleName}`" label="settings">
            <div flex items-center gap-1>
              <div flex items-center gap-1>
                ...
                <span decoration-underline decoration-dashed>
                  {{ t(`settings.pages.modules.${moduleName}.title`) }}
                </span>
              </div>
    ...
    <div flex items-center gap-2 text-sm>
      ...
      <div v-if="Array(pendingResources.values()).length > 0">Preparing resources...</div>
    </div>
    <div
      v-if="Array(pendingResources.values()).length > 0"
      max-w="100%"
      max-h="[calc(50dvh)]"
      w-full
      flex
