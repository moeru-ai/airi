...
var lerp$1 = (a, b, t) => a + t * (b - a);
var unlerp = (a, b, v) => (v - a) / (b - a);
var blerp = (a00, a01, a10, a11, tx, ty) => {
	return lerp$1(lerp$1(a00, a01, tx), lerp$1(a10, a11, tx), ty);
};
var trilerp = (a000, a010, a100, a110, a001, a011, a101, a111, tx, ty, tz) => {
	return lerp$1(blerp(a000, a010, a100, a110, tx, ty), blerp(a001, a011, a101, a111, tx, ty), tz);
};
...
const interpolatorLinear = interpolatorPiecewise(lerp$1);
...
var fixupHueLonger = (arr) => hue(arr, (d) => Math.abs(d) >= 180 || d === 0 ? d : d - 360 * Math.sign(d));
var fixupHueIncreasing = (arr) => hue(arr, (d) => d >= 0 ? d : d + 360);
var fixupHueDecreasing = (arr) => hue(arr, (d) => d <= 0 ? d : d - 360);
...
function average(colors, mode = "rgb", overrides) {
	...
	return def.channels.reduce((res, ch) => {
		let arr = cc.map((c) => c[ch]).filter((val) => val !== void 0);
		if (arr.length) {
			let fn;
			if (isfn$1(overrides)) fn = overrides;
			else if (overrides && isfn$1(overrides[ch])) fn = overrides[ch];
			else if (def.average && isfn$1(def.average[ch])) fn = def.average[ch];
			else fn = averageNumber;
			res[ch] = fn(arr, ch);
		}
		return res;
	}, { mode });
...
var convertDlchToLab65 = ({ l, c, h, alpha }) => {
	...
	let G = (Math.exp(.0435 * c * kCH * kE) - 1) / .075;
...
const serializeRgb = (color) => {
	...
	let g = fixup(color.g);
	let b = fixup(color.b);
	if (color.alpha === void 0 || color.alpha === 1) return `rgb(${r}, ${g}, ${b})`;
	else return `rgba(${r}, ${g}, ${b}, ${twoDecimals(clamp$1(color.alpha))})`;
};
...
const formatHex = (c) => serializeHex(rgb$3(c));
const formatHex8 = (c) => serializeHex8(rgb$3(c));
const formatRgb = (c) => serializeRgb(rgb$3(c));
const formatHsl = (c) => serializeHsl(hsl$1(c));

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/blend.js
var BLENDS = {
	normal: (b, s) => s,
	multiply: (b, s) => b * s,
	screen: (b, s) => b + s - b * s,
	...
	return colors.map((c) => {
		...
		return channels.reduce((res, ch) => {
			if (ch !== "alpha") if (alpha === 0) res[ch] = 0;
				...
				res[ch] = s.alpha * (1 - b.alpha) * s[ch] + s.alpha * b.alpha * fn(b[ch], s[ch]) + (1 - s.alpha) * b.alpha * b[ch];
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/random.js
var rand = ([min, max]) => min + Math.random() * (max - min);
var to_intervals = (constraints) => Object.keys(constraints).reduce((o, k) => {
	let v = constraints[k];
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/map.js
var mapper = (fn, mode = "rgb", preserve_mode = false) => {
	...
	return (color) => {
		...
		let res = (channels || getMode(conv_color.mode).channels).reduce((res, ch) => {
			let v = fn(conv_color[ch], ch, conv_color, mode);
			if (v !== void 0 && !isNaN(v)) res[ch] = v;
			return res;
		}, { mode: conv_color.mode });
		if (!preserve_mode) return res;
		let prep = prepare(color);
		if (prep && prep.mode !== res.mode) return converter(prep.mode)(res);
		return res;
	};
};
var mapAlphaMultiply = (v, ch, c) => {
	if (ch !== "alpha") return (v || 0) * (c.alpha !== void 0 ? c.alpha : 1);
	return v;
};
var mapAlphaDivide = (v, ch, c) => {
	if (ch !== "alpha" && c.alpha !== 0) return (v || 0) / (c.alpha !== void 0 ? c.alpha : 1);
	return v;
};
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/util/normalizePositions.js
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/easing/midpoint.js
var midpoint = (H = .5) => (t) => H <= 0 ? 1 : H >= 1 ? 0 : Math.pow(t, Math.log(.5) / Math.log(H));

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/interpolate/interpolate.js
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/interpolate/splineBasis.js
var mod = (v, l) => (v + l) % l;
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/interpolate/splineNatural.js
var solve = (v) => {
	let i;
	let n = v.length - 1;
	let c = new Array(n);
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/interpolate/splineMonotone.js
var sgn = Math.sign;
var min = Math.min;
var abs = Math.abs;
var mono = (arr) => {
	...
	for (let i = 0; i < n; i++) {
		...
		p.push(i > 0 ? .5 * (arr[i + 1] - arr[i - 1]) * n : void 0);
		yp.push(i > 0 ? (sgn(s[i - 1]) + sgn(s[i])) * min(abs(s[i - 1]), abs(s[i]), .5 * abs(p[i])) : void 0);
	}
	return [
		s,
		p,
...
const interpolatorSplineMonotone2 = (arr) => {
	...
	let n = arr.length - 1;
	let [s, p, yp] = mono(arr);
	p[0] = (arr[1] * 2 - arr[0] * 1.5 - arr[2] * .5) * n;
	p[n] = (arr[n] * 1.5 - arr[n - 1] * 2 + arr[n - 2] * .5) * n;
	yp[0] = p[0] * s[0] <= 0 ? 0 : abs(p[0]) > 2 * abs(s[0]) ? 2 * s[0] : p[0];
	yp[n] = p[n] * s[n - 1] <= 0 ? 0 : abs(p[n]) > 2 * abs(s[n - 1]) ? 2 * s[n - 1] : p[n];
	return interpolator(arr, yp, s);
};
const interpolatorSplineMonotoneClosed = (arr) => {
	...
	let [s, p, yp] = mono(arr);
	p[0] = .5 * (arr[1] - arr[n]) * n;
	p[n] = .5 * (arr[0] - arr[n - 1]) * n;
	let s_m1 = (arr[0] - arr[n]) * n;
	let s_n = s_m1;
	yp[0] = (sgn(s_m1) + sgn(s[0])) * min(abs(s_m1), abs(s[0]), .5 * abs(p[0]));
	yp[n] = (sgn(s[n - 1]) + sgn(s_n)) * min(abs(s[n - 1]), abs(s_n), .5 * abs(p[n]));
	return interpolator(arr, yp, s);
};

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/easing/gamma.js
var gamma = (γ = 1) => γ === 1 ? (t) => t : (t) => Math.pow(t, γ);

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/samples.js
var samples = (n = 2, γ = 1) => {
	let ease = gamma(γ);
...
var fixup_rgb = (c) => {
	const res = {
		mode: c.mode,
		r: Math.max(0, Math.min(c.r !== void 0 ? c.r : 0, 1)),
		g: Math.max(0, Math.min(c.g !== void 0 ? c.g : 0, 1)),
		b: Math.max(0, Math.min(c.b !== void 0 ? c.b : 0, 1))
	};
	if (c.alpha !== void 0) res.alpha = c.alpha;
	return res;
};
var to_displayable_srgb = (c) => fixup_rgb(rgb$2(c));
var inrange_rgb = (c) => {
	return c !== void 0 && (c.r === void 0 || c.r >= 0 && c.r <= 1) && (c.g === void 0 || c.g >= 0 && c.g <= 1) && (c.b === void 0 || c.b >= 0 && c.b <= 1);
};
...
function clampRgb(color) {
	...
	return converter(color.mode)(to_displayable_srgb(color));
}
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/nearest.js
var nearest = (colors, metric = differenceEuclidean(), accessor = (d) => d) => {
	let arr = colors.map((c, idx) => ({
		color: accessor(c),
		i: idx
	}));
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/filter.js
var minzero = (v) => Math.max(v, 0);
var clamp = (v) => Math.max(Math.min(v, 1), 0);
var lerp = (a, b, t) => a === void 0 || b === void 0 ? void 0 : a + t * (b - a);
...
var matrix = (values, mode, preserve_mode = false) => {
	...
	return (color) => {
		...
		for (let i = 0; i < values.length; i++) {
			...
			if (c[ch] === void 0) continue;
			res[ch] = (res[ch] || 0) + values[i] * (c[channels[i % count]] || 0);
		}
		if (!preserve_mode) return res;
		let prep = prepare(color);
...
var filterInvert = (amt = 1, mode = "rgb") => {
	let a = clamp(amt);
	return mapper((v, ch) => ch === "alpha" ? v : lerp(a, 1 - a, v), mode, true);
};
var filterHueRotate = (deg = 0, mode = "rgb") => matrix(matrixHueRotate(deg), mode, true);

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/deficiency.js
...
var deficiency = (lut, t) => {
	...
	if (w > 0 && i < lut.length - 1) {
		let arr_2 = lut[i + 1];
		arr = arr.map((v, idx) => lerp$1(arr[idx], arr_2[idx], w));
	}
	return (color) => {
		let c = prepare(color);
		if (c === void 0) return;
		...
		if (c.alpha !== void 0) ret.alpha = c.alpha;
...
//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/easing/smoothstep.js
var easingSmoothstep = (t) => t * t * (3 - 2 * t);
var easingSmoothstepInverse = (t) => .5 - Math.sin(Math.asin(1 - 2 * t) / 3);

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/easing/smootherstep.js
var smootherstep = (t) => t * t * t * (t * (t * 6 - 15) + 10);

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/easing/inOutSine.js
var inOutSine = (t) => (1 - Math.cos(t * Math.PI)) / 2;

//#endregion
//#region ../../node_modules/.pnpm/culori@4.0.2/node_modules/culori/src/wcag.js
...
const a98 = useMode(definition$26);
const cubehelix = useMode(definition$25);
const dlab = useMode(definition$24);
const dlch = useMode(definition$23);
const hsi = useMode(definition$22);
const hsl = useMode(definition$21);
const hsv = useMode(definition$20);
const hwb = useMode(definition$19);
const itp = useMode(definition$18);
const jab = useMode(definition$17);
const jch = useMode(definition$16);
const lab = useMode(definition$15);
const lab65 = useMode(definition$14);
const lch = useMode(definition$13);
const lch65 = useMode(definition$12);
const lchuv = useMode(definition$11);
const lrgb = useMode(definition$10);
const luv = useMode(definition$9);
const okhsl = useMode(modeOkhsl);
const okhsv = useMode(modeOkhsv);
const oklab = useMode(definition$8);
const oklch = useMode(definition$7);
const p3 = useMode(definition$6);
const prophoto = useMode(definition$5);
const rec2020 = useMode(definition$4);
const rgb = useMode(definition$27);
const xyb = useMode(definition$3);
const xyz50 = useMode(definition$2);
const xyz65 = useMode(definition$1);
const yiq = useMode(definition);

//#endregion
export { toGamut as $, convertHsiToRgb as $n, converter as $r, modeOkhsv as $t, yiq as A, convertJabToJch as An, fixupHueShorter as Ar, definition as At, filterBrightness as B, ParseHwb as Bn, interpolatorLinear as Br, definition$4 as Bt, p3 as C, convertXyz50ToLab as Cn, differenceItp as Cr, formatHsl as Ct, xyb as D, convertLabToXyz50 as Dn, fixupHueDecreasing as Dr, serializeHsl as Dt, rgb as E, convertXyz50ToRgb as En, convertRgbToCubehelix as Er, serializeHex8 as Et, easingSmoothstep as F, convertXyz65ToJab as Fn, convertRgbToLrgb as Fr, convertXyz65ToXyz50 as Ft, filterSaturate as G, convertHsvToRgb as Gn, unlerp as Gr, convertXyz50ToProphoto as Gt, filterGrayscale as H, convertHwbToRgb as Hn, blerp as Hr, convertXyz65ToRec2020 as Ht, easingSmoothstepInverse as I, definition$18 as In, convertXyz65ToA98 as Ir, definition$2 as It, clampChroma as J, parseHslLegacy as Jn, parse as Jr, convertP3ToXyz65 as Jt, filterSepia as K, definition$21 as Kn, parseTransparent as Kr, definition$6 as Kt, filterDeficiencyDeuter as L, convertXyz65ToItp as Ln, convertA98ToXyz65 as Lr, definition$3 as Lt, luminance as M, convertJabToRgb as Mn, convertXyz65ToRgb as Mr, convertRgbToYiq as Mt, inOutSine as N, convertRgbToJab as Nn, convertLrgbToRgb as Nr, definition$1 as Nt, xyz50 as O, definition$16 as On, fixupHueIncreasing as Or, serializeRgb as Ot, smootherstep as P, convertJabToXyz65 as Pn, convertRgbToXyz65 as Pr, convertXyz50ToXyz65 as Pt, inGamut as Q, convertRgbToHsi as Qn, useParser as Qr, parseOklab as Qt, filterDeficiencyProt as R, convertItpToXyz65 as Rn, definition$27 as Rr, convertXybToRgb as Rt, oklch as S, convertRgbToLab as Sn, differenceHyab as Sr, formatHex8 as St, rec2020 as T, convertLabToRgb as Tn, convertCubehelixToRgb as Tr, serializeHex as Tt, filterHueRotate as U, definition$20 as Un, lerp$1 as Ur, definition$5 as Ut, filterContrast as V, convertRgbToHwb as Vn, interpolatorPiecewise as Vr, convertRec2020ToXyz65 as Vt, filterInvert as W, convertRgbToHsv as Wn, trilerp as Wr, convertProphotoToXyz50 as Wt, clampRgb as X, convertHslToRgb as Xn, removeParser as Xr, parseOklch as Xt, clampGamut as Y, convertRgbToHsl as Yn, getMode as Yr, definition$7 as Yt, displayable as Z, definition$22 as Zn, useMode as Zr, definition$8 as Zt, lrgb as _, definition$13 as _n, differenceCmc as _r, mapper as _t, hsi as a, convertOklabToRgb as an, convertXyz65ToLab65 as ar, interpolatorSplineNatural as at, okhsv as b, definition$15 as bn, differenceHueNaive as br, formatCss as bt, hwb as c, convertLrgbToOklab as cn, convertLchToLab as cr, interpolatorSplineBasisClosed as ct, jch as d, definition$11 as dn, average as dr, interpolateWithPremultipliedAlpha as dt, parseRgbLegacy as ei, convertOkhsvToOklab as en, definition$23 as er, samples as et, lab as f, convertLuvToXyz50 as fn, averageAngle as fr, midpoint as ft, lchuv as g, definition$12 as gn, differenceCiede2000 as gr, mapTransferLinear as gt, lch65 as h, convertLuvToLchuv as hn, differenceCie94 as hr, mapTransferGamma as ht, dlch as i, convertOklabToOkhsl as in, convertRgbToLab65 as ir, interpolatorSplineMonotoneClosed as it, contrast as j, definition$17 as jn, definition$26 as jr, convertYiqToRgb as jt, xyz65 as k, convertJchToJab as kn, fixupHueLonger as kr, round as kt, itp as l, definition$9 as ln, convertLabToLch as lr, interpolate as lt, lch as m, convertLchuvToLuv as mn, differenceCie76 as mr, mapAlphaMultiply as mt, cubehelix as n, parseNamed as ni, modeOkhsl as nn, convertLab65ToDlch as nr, interpolatorSplineMonotone as nt, hsl as o, convertOklabToLrgb as on, convertLab65ToRgb as or, interpolatorSplineNaturalClosed as ot, lab65 as p, convertXyz50ToLuv as pn, averageNumber as pr, mapAlphaDivide as pt, nearest as q, parseHsl as qn, parseRgb as qr, convertXyz65ToP3 as qt, dlab as r, named as ri, convertOkhslToOklab as rn, convertDlchToLab65 as rr, interpolatorSplineMonotone2 as rt, hsv as s, convertRgbToOklab as sn, convertLab65ToXyz65 as sr, interpolatorSplineBasis as st, a98 as t, parseHex as ti, convertOklabToOkhsv as tn, definition$24 as tr, gamma as tt, jab as u, definition$10 as un, definition$25 as ur, interpolateWith as ut, luv as v, parseLch as vn, differenceEuclidean as vr, random as vt, prophoto as w, convertRgbToXyz50 as wn, differenceKotsarenkoRamos as wr, formatRgb as wt, oklab as x, parseLab as xn, differenceHueSaturation as xr, formatHex as xt, okhsl as y, definition$14 as yn, differenceHueChroma as yr, blend as yt, filterDeficiencyTrit as z, definition$19 as zn, fixupAlpha as zr, convertRgbToXyb as zt };
//# sourceMappingURL=src-BF1qKgBv.js.map