import { Bt as onUnmounted, Gn as reactive, Ht as openBlock, It as onMounted, J as createElementBlock, K as createBlock, Kn as readonly, N as Fragment, Ot as mergeProps, Sn as withCtx, W as computed, Wt as provide, Xn as shallowRef, Yt as renderSlot, Z as createRenderer, _n as watch, cr as normalizeStyle, dt as getCurrentInstance, fn as useSlots, kt as nextTick, mt as h, or as normalizeClass, q as createCommentVNode, qn as ref, rr as unref, rt as defineComponent, tr as toValue, vn as watchEffect, zn as isRef } from "./vue.runtime.esm-bundler-DOfC7S2N.js";
import { $r as MathUtils, $t as DirectionalLightHelper, Bo as SphereGeometry, Bs as Vector3, Do as Scene, Dr as Line3, Er as Line, Go as SpotLightHelper, Hn as HemisphereLightHelper, I as BackSide, J as BufferAttribute, O as ArrowHelper, Or as LineBasicMaterial, Vi as Object3D, Xr as Material, Y as BufferGeometry, _a as Quaternion, _r as Layers, _s as Triangle, ai as MeshBasicMaterial, c as three_module_exports, ct as Color, ea as PCFSoftShadowMap, ii as Mesh, l as ACESFilmicToneMapping, lo as Ray, na as PerspectiveCamera, ni as Matrix4, nn as DoubleSide, o as WebGLRenderer, ra as Plane, sa as PointLightHelper, st as Clock, uo as Raycaster, xn as Float32BufferAttribute, zo as Sphere, zs as Vector2 } from "./three.module-Cub_nmrh.js";
import { _ as tryOnScopeDispose, b as whenever, d as useWindowSize, f as createEventHook, g as refDebounced, h as promiseTimeout, i as useElementSize, l as useMemory, n as useAsyncState, o as useFps, p as createInjectionState, r as useDevicePixelRatio, t as unrefElement, u as useRafFn, y as useTimeout } from "./core-DDDqR7YV.js";
import { t as setupDevToolsPlugin } from "./dist-w_lTHz18.js";
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/html-event.js
var HtmlEvent = class {
	nativeEvent;
	NONE = 0;
	CAPTURING_PHASE = 1;
	...
	getFromNative(key, defaultValue) {
		if (key in this.nativeEvent) return this.nativeEvent[key];
		return defaultValue;
	}
};

//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/event.js
var helperVector = new Vector3();
var PointerEvent = class PointerEvent extends HtmlEvent {
	type;
	bubbles;
	internalPointer;
	intersection;
	...
	get ray() {
		...
		switch (this.intersection.details.type) {
			case "screen-ray":
			case "ray":
			case "sphere": return this._ray = new Ray(this.intersection.pointerPosition, new Vector3(0, 0, -1).applyQuaternion(this.intersection.pointerQuaternion));
			case "lines": return this._ray = new Ray(this.intersection.details.line.start, this.intersection.details.line.end.clone().sub(this.intersection.details.line.start).normalize());
		}
	}
	_intersections = [];
	get intersections() {
		if (this._intersections == null) this._intersections = [{
			...this.intersection,
...
var WheelEvent = class WheelEvent extends PointerEvent {
	get deltaX() {
...
function emitPointerEventRec(baseEvent, currentObject) {
	...
	if (listeners != null && listeners.length > 0) {
		...
		for (let i = 0; i < length && !event.stoppedImmediate; i++) listeners[i](event);
	}
	if (baseEvent.stopped) return;
...
function getObjectListeners(object, forEvent) {
	if (object._listeners != null && forEvent in object._listeners) return object._listeners[forEvent];
	let handler;
	if (object.isVoidObject && forEvent === "click" && object.parent?.__r3f != null) handler = object.parent.__r3f.root.getState().onPointerMissed;
	if (object.__r3f != null) handler = object.__r3f.handlers[r3fEventToHandlerMap[forEvent]];
	if (handler == null) return;
	return [handler];
}

//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/intersections/intersector.js
var VoidObjectGeometry = new SphereGeometry(1e10);
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/intersections/utils.js
...
function isPointerEventsAllowed(hasListener, pointerEvents, pointerEventsType) {
	...
	if (pointerEvents === "listener" && !hasListener) return false;
	if (pointerEventsType === "all") return true;
	if (typeof pointerEventsType === "function") return ({ id, type, state }) => pointerEventsType(id, type, state);
	let value;
	let invert;
	...
	if (Array.isArray(value)) return (pointer) => invertIf(value.includes(pointer.type), invert);
...
function intersectPointerEventTargets(type, object, pointers, parentHasListener = false, parentPointerEvents, parentPointerEventsType, parentPointerEventsOrder) {
	...
	if (length === 1) {
		if (isAllowed === true || typeof isAllowed === "function" && isAllowed(pointers[0])) filterAndInteresct(pointers[0], object, pointerEventsOrDefault, pointerEventsType, pointerEventsOrder);
	} else if (isAllowed === true) for (let i = 0; i < length; i++) filterAndInteresct(pointers[i], object, pointerEventsOrDefault, pointerEventsType, pointerEventsOrder);
	else if (typeof isAllowed === "function") for (let i = 0; i < length; i++) {
		const pointer = pointers[i];
		if (!isAllowed(pointer)) continue;
		filterAndInteresct(pointer, object, pointerEventsOrDefault, pointerEventsType, pointerEventsOrder);
	}
	if (object.children.length === 0 || object.intersectChildren === false) return;
	const descendants = object.interactableDescendants ?? object.children;
	const descendantsLength = descendants.length;
	for (let i = 0; i < descendantsLength; i++) intersectPointerEventTargets(type, descendants[i], pointers, hasListener, pointerEvents, pointerEventsType, pointerEventsOrder);
}
function hasObjectListeners(type, object) {
	if (object.ancestorsHaveListeners) return true;
	if (type === "pointer" && object.ancestorsHavePointerListeners) return true;
	if (type === "wheel" && object.ancestorsHaveWheelListeners) return true;
	if (object.__r3f != null && object.__r3f?.eventCount > 0) {
		if (type === "wheel" && object.__r3f["handlers"]["onWheel"] != null) return true;
		if (type === "pointer" && Object.keys(object.__r3f["handlers"]).some((key) => key != "onWheel")) return true;
	}
	if (object._listeners == null) return false;
	if (type === "wheel") {
	...
	for (let i = 0; i < length; i++) {
		...
		if (entry[0] === "wheel") continue;
		if (!listenerNames.includes(entry[0])) continue;
		if (entry[1] != null && entry[1].length > 0) return true;
	}
	return false;
}
function filterAndInteresct({ intersector, options }, object, pointerEvents, pointerEventsType, pointerEventsOrder) {
	if (options.filter?.(object, pointerEvents, pointerEventsType, pointerEventsOrder) === false) return;
...
function getDominantIntersectionIndex(intersections, pointerEventsOrders, { customSort: compare = defaultSort } = {}, filter) {
	...
	for (let i = 0; i < length; i++) {
		...
		if (filter?.(newIntersection) === false) continue;
...
function defaultSort(i1, pointerEventsOrder1 = 0, i2, pointerEventsOrder2 = 0) {
	if (pointerEventsOrder1 != pointerEventsOrder2) return pointerEventsOrder2 - pointerEventsOrder1;
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/pointer.js
...
Object3D.prototype.releasePointerCapture = function(pointerId) {
	...
	if (pointer == null || !pointer.hasCaptured(this)) return;
...
var Pointer = class {
	...
	*/
	...
	setCapture(object) {
		if (this.pointerCapture?.object === object) return;
	...
	*/
	...
	setEnabled(enabled, nativeEvent, commit = true) {
		...
		if (commit) this.commit(nativeEvent, false);
	}
	computeIntersection(type, scene, nativeEvent) {
		if (this.pointerCapture != null) return this.intersector.intersectPointerCapture(this.pointerCapture, nativeEvent);
	...
	commit(nativeEvent, emitMove) {
		...
		if (prevIntersection != null && prevIntersection.object != intersection?.object) emitPointerEvent(new PointerEvent("pointerout", true, nativeEvent, this, prevIntersection, camera));
		...
		if (intersection != null && prevIntersection?.object != intersection.object) emitPointerEvent(new PointerEvent("pointerover", true, nativeEvent, this, intersection, camera));
		...
		if (emitMove && intersection != null) emitPointerEvent(new PointerEvent("pointermove", true, nativeEvent, this, intersection, camera));
		...
		if (!this.wasMoved && this.intersector.isReady()) {
			...
			for (let i = 0; i < length; i++) this.onFirstMove[i](camera);
	...
	*/
	...
	up(nativeEvent) {
		...
		const camera = this.getCamera();
		if (isClicked && nativeEvent.button === contextMenuButton) emitPointerEvent(new PointerEvent("contextmenu", true, nativeEvent, this, this.intersection, camera));
	...
	wheel(scene, nativeEvent, useMoveIntersection = false) {
		...
		if (!useMoveIntersection) this.wheelIntersection = this.computeIntersection("wheel", scene, nativeEvent);
...
function computeEnterLeave(currentObject, targetAllAncestors, targeDiffRemovedAncestors, targetDiffAddedAncestors) {
	...
	if (index != -1) targeDiffRemovedAncestors.splice(index, 1);
	else targetDiffAddedAncestors.push(currentObject);
...
function getIsClicked(pointerButtonsPressTime, objectButtonsDownTime, button, buttonUpTime, clickThresholdMs) {
	if (objectButtonsDownTime == null) return false;
	const objectButtonPressTime = objectButtonsDownTime.get(button);
	if (objectButtonPressTime == null) return false;
	if (buttonUpTime - objectButtonPressTime > clickThresholdMs) return false;
	if (objectButtonPressTime != pointerButtonsPressTime.get(button)) return false;
	return true;
}

//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/utils.js
...
function getClosestUV(target, point, mesh) {
	...
	if (uv == null || !(uv instanceof BufferAttribute)) return false;
	...
	if (clostestDistance == null) return false;
...
function loopThroughTriangles(mesh, fn) {
	...
	if (mesh.geometry.index != null) {
		...
		for (let i = start; i < end; i += 3) fn(index.getX(i), index.getX(i + 1), index.getX(i + 2));
		return;
	}
	...
	for (let i = start; i < end; i += 3) fn(i, i + 1, i + 2);
}

//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/intersections/lines.js
var invertedMatrixHelper$2 = new Matrix4();
var lineHelper = new Line3();
var scaleHelper$2 = new Vector3();
var planeHelper$2 = new Plane();
var rayHelper = new Ray();
var point2Helper$2 = new Vector2();
var defaultLinePoints = [new Vector3(0, 0, 0), new Vector3(0, 0, 1)];

//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/intersections/ray.js
var invertedMatrixHelper$1 = new Matrix4();
var scaleHelper$1 = new Vector3();
var NegZAxis = new Vector3(0, 0, -1);
var planeHelper$1 = new Plane();
var point2Helper$1 = new Vector2();
...
var ScreenRayIntersector = class {
	...
	intersectPointerCapture({ intersection, object }, nativeEvent) {
		...
		if (details.type != "screen-ray") throw new Error(`unable to process a pointer capture of type "${intersection.details.type}" with a camera ray intersector`);
		if (!this.startIntersection(nativeEvent)) return intersection;
		...
		if (point == null) return intersection;
		intersection.object.updateWorldMatrix(true, false);
		computeIntersectionWorldPlane(this.viewPlane, intersection, intersection.object.matrixWorld);
		let uv = intersection.uv;
		if (intersection.object instanceof Mesh && getClosestUV(point2Helper$1, point, intersection.object)) uv = point2Helper$1.clone();
		return {
			...intersection,
			details: {
				...details,
	...
	startIntersection(nativeEvent) {
		...
		from.matrixWorld.decompose(this.fromPosition, this.fromQuaternion, scaleHelper$1);
	...
	finalizeIntersection(scene) {
		...
		if (intersection == null) return voidObjectIntersectionFromRay(scene, this.raycaster.ray, (_point, distance) => ({
			type: "screen-ray",
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/intersections/sphere.js
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/pointer/index.js
...
//#endregion
//#region ../../node_modules/.pnpm/@pmndrs+pointer-events@6.6.28/node_modules/@pmndrs/pointer-events/dist/forward.js
function htmlEventToCoords(element, e, target) {
	if (!(e instanceof globalThis.MouseEvent)) return target.set(0, 0);
...
function forwardHtmlEvents(fromElement, getCamera, scene, options) {
	return forwardEvents(fromElement, typeof getCamera === "function" ? getCamera : () => getCamera, scene, htmlEventToCoords.bind(null, fromElement), fromElement.setPointerCapture.bind(fromElement), (pointerId) => {
		if (fromElement.hasPointerCapture(pointerId)) fromElement.releasePointerCapture(pointerId);
	}, {
...
function forwardEvents(from, getCamera, scene, toCoords, setPointerCapture, releasePointerCapture, options = {}) {
	...
	const getInnerPointer = (event, eventType) => {
		...
		if (innerPointer != null) return innerPointer;
	...
	const onEvent = (type, event) => {
		...
		if (type === "move") latestMoveEventMap.set(pointer, event);
		if (type === "wheel") latestWheelEventMap.set(pointer, event);
		if (options.batchEvents ?? true) eventList.push({
			type,
			event
		});
		else emitEvent(type, event, pointer);
	};
	...
	return {
		...
		update() {
			...
			eventList.length = 0;
			if (options.intersectEveryFrame ?? false) for (const [pointer, event] of latestMoveEventMap.entries()) {
				if (movedPointerList.includes(pointer)) continue;
				pointer.move(scene, event);
			}
...
//#endregion
//#region ../../node_modules/.pnpm/@tresjs+core@5.5.0_three@0.183.1_vue@3.5.29_typescript@5.9.3_/node_modules/@tresjs/core/dist/tres.js
/**
* name: @tresjs/core
* version: v5.5.0
* (c) 2026
...
var createTypeGuard = (property) => (value) => isObject(value) && property in value && !!value[property];
/**
...
var Context_default = /* @__PURE__ */ defineComponent({
	__name: "Context",
	props: {
		camera: {
			type: null,
...
var TresCanvas_default = /* @__PURE__ */ defineComponent({
	__name: "TresCanvas",
	props: {
		camera: {
			type: null,
...
var template_compiler_options_default = { template: { compilerOptions: { isCustomElement: (tag) => (/^Tres[A-Z]/.test(tag) || tag.startsWith("tres-")) && !whitelist.includes(tag) || tag === "primitive" } } };
...
//#endregion
export { isTresPrimitive as A, useCameraManager as B, isOrthographicCamera as C, isTresCamera as D, isScene as E, normalizeColor as F, useTres as G, useLoader as H, normalizeVectorFlexibleParam as I, vDistanceTo as J, useTresContext as K, registerTresDevtools as L, logError as M, logMessage as N, isTresInstance as O, logWarning as P, src_default as R, isObject3D as S, isProd as T, useLoop as U, useGraph as V, useRendererManager as W, vLog as X, vLightHelper as Y, isGroup as _, buildGraph as a, isMaterial as b, disposeObject3D as c, isCamera as d, isClassInstance as f, isFog as g, isCopyable as h, TresRendererError as i, isVectorLike as j, isTresObject as k, extend as l, isColorRepresentation as m, DevtoolsMessenger as n, catalogue as o, isColor as p, useTresContextProvider as q, TresCanvas_default as r, component_default as s, Context_default as t, isBufferGeometry as u, isLayers as v, isPerspectiveCamera as w, isMesh as x, isLight as y, template_compiler_options_default as z };
//# sourceMappingURL=tres-CFl1hQ_Q.js.map