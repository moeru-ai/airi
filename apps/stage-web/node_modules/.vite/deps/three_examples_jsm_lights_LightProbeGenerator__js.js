import { Ai as NoColorSpace, Bn as HalfFloatType, Br as LinearSRGBColorSpace, Bs as Vector3, Ho as SphericalHarmonics3, Sn as FloatType, To as SRGBColorSpace, Tr as LightProbe, Wt as DataUtils, ct as Color, qs as WebGLCoordinateSystem } from "./three.module-Cub_nmrh.js";

//#region ../../node_modules/.pnpm/three@0.183.1/node_modules/three/examples/jsm/lights/LightProbeGenerator.js
/**
...
var LightProbeGenerator = class {
	...
	static fromCubeTexture(cubeTexture) {
		...
		for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
			...
			for (let i = 0, il = data.length; i < il; i += 4) {
				...
				const col = -1 + (pixelIndex % imageWidth + .5) * pixelSize;
				const row = 1 - (Math.floor(pixelIndex / imageWidth) + .5) * pixelSize;
				switch (faceIndex) {
					case 0:
						coord.set(-1, row, -col);
						break;
	...
	*/
	static async fromCubeRenderTarget(renderer, cubeRenderTarget) {
		...
		let data;
		if (renderer.isWebGLRenderer) if (dataType === FloatType) data = new Float32Array(imageWidth * imageWidth * 4);
		else if (dataType === HalfFloatType) data = new Uint16Array(imageWidth * imageWidth * 4);
		else data = new Uint8Array(imageWidth * imageWidth * 4);
		for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
			if (renderer.isWebGLRenderer) await renderer.readRenderTargetPixelsAsync(cubeRenderTarget, 0, 0, imageWidth, imageWidth, data, faceIndex);
			else data = await renderer.readRenderTargetPixelsAsync(cubeRenderTarget, 0, 0, imageWidth, imageWidth, 0, faceIndex);
			const pixelSize = 2 / imageWidth;
			for (let i = 0, il = data.length; i < il; i += 4) {
				let r, g, b;
				...
				const pixelIndex = i / 4;
				const col = (1 - (pixelIndex % imageWidth + .5) * pixelSize) * flip;
				const row = 1 - (Math.floor(pixelIndex / imageWidth) + .5) * pixelSize;
				switch (faceIndex) {
					case 0:
