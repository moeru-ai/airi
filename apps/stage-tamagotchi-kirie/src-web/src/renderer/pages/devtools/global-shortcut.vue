...
import { useHostGlobalShortcuts } from '@proj-airi/stage-host-context'
...
async function refreshList() {
  try {
    active.value = await shortcuts.list()
  }
  catch (error) {
...
async function handleRegister() {
  ...
  busy.value = true
  try {
    lastResult.value = await shortcuts.register({
      id: form.id.trim(),
      accelerator,
      scope: 'global',
...
onMounted(async () => {
  disposeTriggerListener = shortcuts.onTriggered((payload) => {
