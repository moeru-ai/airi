          ...
          auth: {
            profile: {
              ...
              emailChange: {
                ...
                accepted: { changed: 'Email changed.', current: 'Sent to current email.' },
...
describe('profile email change', () => {
  ...
  it('tracks a native request from the current email classification only', async () => {
    ...
    await expect.element(screen.getByText('private-target@example.com').first()).toBeVisible()
    await expect.element(screen.getByText('Unverified').first()).toBeVisible()
