...
type EmailChangeViewState
  = { status: 'idle' }
    | { status: 'submitting' }
    | { status: 'accepted', destination: 'changed' | 'current' }
...
interface Emits {
  submitted: [result: { immediate: boolean, newEmail: string }]
}
...
const acceptedMessage = computed(() => {
  ...
  if (state.value.destination === 'changed')
    return t('server.auth.profile.emailChange.accepted.changed')

  return t('server.auth.profile.emailChange.accepted.current')
})
...
async function submitEmailChange(targetEmail: string) {
  ...
  callbackURL.searchParams.set('api_server_url', props.apiServerUrl)
  ...
  try {
    ...
    const immediate = !props.emailVerified
    state.value = {
      status: 'accepted',
      destination: immediate ? 'changed' : 'current',
    }
    if (!immediate)
      startResendCooldown()
    emit('submitted', { immediate, newEmail: targetEmail })
  }
  catch (error) {
    if (!isCurrentRequest(generation))
      return
...
<template>
  ...
  >
    ...
    >
      ...
      <Button
        v-if="state.destination !== 'changed'"
        type="button"
        :class="[
          'w-full py-2',
          'flex items-center justify-center',
