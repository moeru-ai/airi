...
describe('email change section', () => {
  ...
  // https://github.com/moeru-ai/airi/pull/2473#discussion_r3943769451
  it('preserves the selected API server in the native callback URL', async () => {
    ...
    expect(requestBody(fetchMock, 0)).toEqual({
      body: {
        callbackURL: `${window.location.origin}/ui/profile?email_change=processed&api_server_url=https%3A%2F%2Fauth.airi.test`,
  ...
  it('finishes an unverified email change without offering an invalid resend', async () => {
    const onSubmitted = vi.fn()
    const screen = await renderSection({
  ...
  it('finishes a placeholder email change without offering an invalid resend', async () => {
    const onSubmitted = vi.fn()
    const screen = await renderSection({
      email: '123@steam.placeholder.local',
      emailVerified: false,
      onSubmitted,
    })
    ...
    await expect.element(screen.getByRole('status')).toHaveTextContent('Your email address was changed.')
    expect(document.querySelector('button')).toBeNull()
    expect(onSubmitted).toHaveBeenCalledWith({
      immediate: true,
      newEmail: 'new@example.com',
