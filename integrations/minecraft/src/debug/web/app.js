...
class ConversationPanel {
  constructor(client) {
    this.client = client
    this._mkSession = () => ({ greyed: false, messages: [] })
  ...
  renderUserSection(section, turnNum) {
    ...
    if (['ACTION_QUEUE', 'CONTEXT', 'ERROR_BURST', 'NO_ACTION_BUDGET', 'STATE'].includes(section.tag)) {
...
class QueuePanel {
  ...
  render() {
    const { processing, queue } = this.data
...
class ReflexPanel {
  ...
  render() {
    ...
    const { activeBehaviorId, context, mode } = this.state
