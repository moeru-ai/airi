...
本文档介绍了如何在 AIRI 中配置 Novita AI 作为大模型服务商。由于此类提供商原生支持 OpenAI 格式的 API，配置过程非常标准且直接。

::: info 为什么选择 Novita AI？
拥有海量的文本和图像模型支持，且高性能 GPU 算力成本极低，非常适合开发者和初创团队快速上线应用。
:::

## 第一步：获取 API 密钥

1. 登录你的 [Novita AI 管理控制台](https://novita.ai/dashboard)。
...
请在 AIRI 的 **设置 -> 服务来源 -> Novita AI** 页面中按以下说明填写：

### 1. 基础设置 (Basic)
* **API 密钥**: 填入你在 Novita AI 后台生成的 API 令牌。
    * *提示：点击右侧的刷新图标可以清空输入。*

### 2. 高级设置 (Advanced)
点击 **Advanced** 箭头展开隐藏选项：
* **Base URL**: `https://api.novita.ai/v1`。默认情况下不需要更改。
...
如果你需要手动测试 Novita AI 的 API 连通性，可以使用以下 cURL 命令进行调试：
