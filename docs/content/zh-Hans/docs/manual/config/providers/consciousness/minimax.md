...
MiniMax 提供兼容 OpenAI 格式的聊天 API。完成本页配置后，AIRI 可以在“意识”中使用 MiniMax 提供的模型。

::: info 为什么选择 MiniMax？
如果你希望在 AIRI 中使用 MiniMax 模型，或已经拥有 MiniMax API Key，可以直接选择此服务商。
:::
...
2. 将 API Key 粘贴到基础设置。
3. 保留默认 Base URL：`https://api.minimax.chat/v1`。
