...
- 其他使用问题也欢迎在 AIRI 的 Discord、微信群或 QQ 群中与社区交流；在 Discord 中可 @0x_selenic_dove。
- 祝您玩的开心！AwA
:::
...
::: info VRM 交互与视角
内置舞台中的位置、旋转、相机距离与注视方向会保存到当前设置。模型是否支持点击交互及能触发哪些表情，取决于 VRM 模型自身包含的表达式。
:::
...
“连接”用于配置 AIRI 的服务通道。您可以设置 WebSocket 地址，并在需要加密传输时启用 TLS。桌面端还可选择仅本机访问、允许局域网访问或填写高级主机名，并设置访问令牌；页面会提供二维码，方便其他设备连接。仅在受信任的网络中开放局域网访问，并妥善保管访问令牌。
...
![AIRI 开发者设置界面](image-10.png)

::: info 高级功能说明
该部分内容大多为英文，而且属于一般用不到的高级功能，因此下面的介绍仅供参考。

网页版的开发者页面与桌面端不同；下面仅说明桌面端。
:::

第一个方框及其相关选项：

- 点击「**打开**」以打开内置浏览器开发者工具窗口。
- 「Markdown 压力测试」会在独立窗口渲染高负载 Markdown，用于检查渲染性能和样式。
- 「IO Tracer」打开 I/O 诊断窗口，用于观察调试期间的输入输出事件。
- 「Lag 可视化」打开性能可视化页面，用于检查界面卡顿和帧时间。
- 「是否开启舞台切换动画」可控制开关舞台切换动画；关闭后可减少切换时的动画效果。
- 「是否使用页面特定过场动画」可控制开关每个页面的专属过场动画；关闭舞台切换动画时，该选项也会不可用。

##### > useMagicKeys 工具

::: info useMagicKeys
这是用于调试键盘快捷键状态的占位页面，当前没有可供普通用户操作的界面。
:::
...
此诊断页可手动检查更新、下载可用更新，并在下载完成后重启安装。下方会显示当前版本、平台、架构、更新通道、更新源地址和日志位置等信息；它用于排查更新问题，普通更新可从“关于”窗口完成。

##### > Screen Capture

译为「屏幕截图」。

如果您还没有授予系统级的屏幕捕获权限，那么首先会看到下图这样的权限提示窗口。完成授权后，才可以开始捕获任意应用程序窗口或者整个屏幕。
...
- 「devices」——列出系统报告的可捕获设备；选择后可使用「share device」创建捕获流。可用设备取决于操作系统和权限。
- 「refetch」——重新读取应用窗口、显示器和设备列表。连接或关闭显示器、打开新窗口，或权限状态改变后可点击它刷新列表。
...
这是开发者用来测试全局快捷键注册、注销和触发事件的页面。它与“系统 → 窗口快捷方式”中面向普通用户的 Spotlight 快捷键设置不同；如只需修改 Spotlight 快捷键，请在“窗口快捷方式”页面操作。

![AIRI Global Shortcut 工具界面](image-12.png)
