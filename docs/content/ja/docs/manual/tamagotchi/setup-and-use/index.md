...
- **音声合成 (TTS)**: AIRI に返信を読み上げさせます。**[モジュール] → [音声]** でモデルと声を選択します。
- **音声認識 (ASR/STT)**: マイクの音声をテキストに変換します。**[モジュール] → [聴覚]** でモデルを選択します。
- **アート作成**: 画像生成サービスを設定し、**[モジュール] → [アート作成]** で使用します。

初期構成ガイドをスキップする場合は、最初にチャット プロバイダーを設定します。API キーまたはログイン情報を入力し、必要に応じてベース URL やリージョンなどの詳細フィールドも入力します。**[Ping API]** が表示される場合は接続を確認します。その後、**[モジュール] → [意識]** でプロバイダーとモデルを選択し、メッセージを送信して AIRI が応答することを確認します。
