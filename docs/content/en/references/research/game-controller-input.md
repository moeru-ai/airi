...
The sample starts HID reconnection after `gamepadconnected`. AIRI cannot copy this trigger. The tested Electron 41 build did not emit that event for Bluetooth DualSense.
...
Test the current Electron 43 build before AIRI adds a permanent macOS fallback. Keep WebHID for device-specific features.
...
The test that motivated this research used Electron 41. [Electron 41 uses Chromium 146](https://www.electronjs.org/blog/electron-41-0#stack-changes).

The current workspace selects Electron `^43.4.1`. Electron 43 includes the newer PlayStation GameController path. Repeat the Bluetooth test on the current build.
...
3. Test the current Electron 43 build before changing the input architecture. [Electron 43 uses Chromium 150](https://www.electronjs.org/blog/electron-43-0#stack-changes), which contains the PlayStation GameController change.
4. Keep WebHID for device-specific data. Use it for common input only if the current Gamepad API test fails. Apple supports USB and Bluetooth DualSense connections on Mac. See [Apple's controller connection guide](https://support.apple.com/en-us/111100).
...
## Source provenance for the supplied DualSense code
...
The project confirmed that an AIRI contributor authored this implementation. The author supplied it for inclusion in AIRI.

The [source repository](https://github.com/nondebug/dualsense) does not contain a license file. GitHub also shows no repository license.

The AIRI package license controls redistribution of this package. The source repository does not grant a separate reuse license.

Keep this provenance note with the package. It records why AIRI can include code from a source repository without a license file.
