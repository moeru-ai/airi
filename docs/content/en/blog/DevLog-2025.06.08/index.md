...
Hello everyone, here's LemonNeko, one of maintainer of Moeka. Today's DevLog is talking about: Let Live2D model of Moeka Tamagotchi to focus position.

大家好，我是 LemonNeko，Moeka 的维护者之一，由我来为大家带来今天的 DevLog：让 Moeka 桌宠的 Live2D 模型可以注视鼠标位置。
...
The blue box is the screen, the pink box is the Moeka window, the purple arrow is the cursor, we define:

蓝色框是屏幕，粉色是 Moeka 的窗口，紫色箭头是鼠标，我们定义：

- The screen size is: `A x B`<br>屏幕高宽为：`A x B`
- The position of Moeka window is: `(E, F)`<br>Moeka 窗口左上角位置为：`(E, F)`
- The size of Moeka window is: `C x D`<br>Moeka 窗口大小为：`C x D`
- The position of cursor is: `G, H`<br>鼠标位置是：`G, H`

Then the relative position of cursor to window is: `(G - E, H - F)`

那么鼠标的位置在 Moeka 窗口中的位置应当是：`(G - E, H - F)`
