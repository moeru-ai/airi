...
Tamagotchi is the name of the Desktop version of Moeka, where you can have it
...
If you are going to use Moeka for VTuber streaming, it's fine to not having
...
inside Moeka to fade out when the mouse is hovering over the window, and
...
Moeka to stay with me with any applications I use now without worrying about
to disable or organize the order of the windows. Everyday when I developing
Moeka, no matter it's Web version or Desktop version, I will always open her
...
We know that configuring Moeka is quite complex now (but still easy if comparing
...
have a much better experience when you first time using Moeka.

They wrote a blog after the Pull Request being merged to share the
experience contributing Project Moeka: [Moekaプロジェクトに参加した話 - YAMA-blog](https://yama-pro.blog/posts/airi/)
...
With `turborepo`, the time it requires to build Moeka **reduced from 4 minutes
to 25 seconds in average**.

### Now Nix is supported

Thanks to [@Weathercold (Weathercold)](https://github.com/Weathercold), we now
have a Nix flake to build Moeka, which is a great addition to the
