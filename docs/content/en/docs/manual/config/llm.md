...
description: Configuring chat providers and models for Moeka
---

The chat model is Moeka's “brain.” To generate text replies, Moeka needs either a provider account with chat-model access or a running local model service.
...
- For a local provider, its model service is running and reachable from the device running Moeka.
...
Moeka loads the model list when the provider supports it. If the list cannot be loaded and the field accepts custom input, enter the exact model ID from the provider's official documentation.
...
Check the API Key, Base URL, account quota, and network connection. For a local service, verify that it is running and that Moeka can reach the configured address.

### Moeka does not reply
...
After chat works, continue to [Configure voice input and output](./audio.md) to let Moeka speak or use microphone input.
