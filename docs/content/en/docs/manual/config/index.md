...
description: Configure Chat, Vision, Speech, Transcription, and Artistry providers for Project Moeka
---

To chat with Moeka, configure at least one Chat provider and chat model. Speech synthesis (TTS) adds voice output, while automatic speech recognition (ASR/STT) adds microphone input. Voice input and output are optional and can be configured independently.

## Configure the minimum required setup

1. Open Moeka’s **Settings → Providers**.
...
4. Send a message to confirm that Moeka can reply.
...
* **[Configure Web Search](./web-search.md)**: Use Tavily to let Moeka search the Internet for the latest information when needed.
* **Providers**: Expand **Providers** in the sidebar and select **Chat**, **Vision**, **Speech**, **Transcription**, or **Artistry**. Provider pages save credentials; module pages select which provider and model Moeka actively uses.
...
After configuring a provider, you can also change Moeka's theme or switch its display model. The current model selector supports Live2D, Spine, VRM, MMD, and Tachie.
...
Compress the complete Live2D model folder as a ZIP file. Moeka ignores VTube Studio's `items_pinned_to_model.json` metadata during import, so you do not need to remove it manually.
:::

<br />

::: tip Model changes
The stage observes model-setting changes and reloads the selected renderer automatically. If an imported model itself fails to load, check its package structure and assets before restarting Moeka.
:::
<br />
