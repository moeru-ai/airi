...
description: Configure the Chinese mainland version of MiniMax in Moeka as a chat provider
is_openai_compatible: true
---

This page applies to API Keys created on the MiniMax open platform in mainland China. MiniMax provides a chat API compatible with the OpenAI format; once configured, Moeka can use its models in Consciousness.
...
If the API check fails, verify the API key, available credit or quota, rate limits, and network connection. If Moeka cannot load the model list, enter the exact model ID provided by MiniMax manually on the **Consciousness** page.
