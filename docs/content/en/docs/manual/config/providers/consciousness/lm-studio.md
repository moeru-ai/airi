...
description: Configuring chat models for Moeka using local LM Studio services
---
...
3. If Moeka and LM Studio run on different devices, enable **Serve on Local Network** in LM Studio, or bind the server to a non-loopback address.
4. If Moeka runs on another device, use the LAN address of the LM Studio device in Moeka.
...
When Moeka cannot connect, make sure that the Local Server is running and that the port matches the Base URL. If Moeka and LM Studio run on different devices, use a LAN address that the Moeka device can reach.
