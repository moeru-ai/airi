...
description: Configuring the Amazon Bedrock Chat Model in Moeka
---
...
Do not expose the Bedrock API key. Store it only in Moeka's provider settings and revoke it when no longer needed.
:::
...
2. Confirm that the AWS account has access to the target model in that region. The Moeka form has no custom endpoint field.

## Verify configuration

1. Wait for Moeka's automatic validation after entering the API key and region.
