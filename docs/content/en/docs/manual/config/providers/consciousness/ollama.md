...
description: Configuring the chat model for Moeka using a local Ollama service
---
...
2. Keep the default Base URL, `http://localhost:11434/v1/`. If Ollama runs on another device, enter a URL that the device running Moeka can reach.
...
If Moeka cannot connect, confirm that Ollama is running and that the Base URL uses the correct port. If Moeka and Ollama run on different devices, use a LAN address reachable from the Moeka device and expose the service only on a trusted network.

::: warning Moeka cannot connect to local Ollama
If Ollama is running but Moeka reports a CORS error, add Moeka's exact origin to `OLLAMA_ORIGINS` and restart Ollama. Do not use a wildcard or expose Ollama to the public internet as a troubleshooting shortcut.
:::
