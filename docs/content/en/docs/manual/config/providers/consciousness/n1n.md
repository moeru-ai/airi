---
title: n1n
description: Configuring the n1n chat model in Moeka
---

n1n provides API-compatible chat model access in Moeka.

::: info Why choose n1n?
If you use n1n's model service, you can fill in its service address and account credentials in Moeka.
:::
...
When verification fails, check the service address, API Key, and deployer's access policy. If the service allows anonymous access, follow the deployer's instructions to leave the API Key blank and confirm that the address is accessible from the device running Moeka.
