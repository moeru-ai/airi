...
Alibaba Cloud NLS provides AIRI with real-time speech-to-text (ASR). After completing the configuration, select Alibaba Cloud NLS under **Settings → Modules → Hearing** and test the microphone input.
...
2. Select Alibaba Cloud NLS and an audio input device under **Settings → Modules → Hearing**.
