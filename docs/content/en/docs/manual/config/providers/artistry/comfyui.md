...
description: Connect local ComfyUI workflows to Moeka
---

ComfyUI lets Moeka use image-generation workflows from your local machine or a trusted LAN. Configure and activate the workflow on the provider page, then select **ComfyUI (Local)** under **Settings → Modules → Artistry**.
...
1. Start ComfyUI. Moeka connects to `http://localhost:8188` by default.
2. Prepare an image workflow that can be executed directly in ComfyUI, and export its API workflow JSON from ComfyUI.
3. If Moeka and ComfyUI are not on the same device, confirm that the address can be accessed from the device where Moeka is located.
...
3. Click **Test** to confirm that Moeka can read the ComfyUI service status.
4. Upload the API workflow JSON in the Workflow area, enter a name, and select the input fields that Moeka may control.
5. Save the workflow and make it the active workflow.

## Verify configuration

1. In **Settings → Modules → Consciousness**, select a chat model that supports tool calling. Moeka needs the model to invoke the Artistry tool before ComfyUI receives a generation task.
2. Open **Settings → Modules → Artistry** and select **ComfyUI (Local)**.
3. Return to chat and ask Moeka to generate a non-sensitive image.
...
- **The browser reports a cross-origin error:** Restart ComfyUI with the CORS options shown on Moeka's ComfyUI provider page.
