...
description: Configuring Replicate image generation in Moeka
---

Replicate lets Moeka generate images with cloud-hosted models. Configure it under **Settings → Providers → Artistry**, then enable it under **Settings → Modules → Artistry**.
...
3. Enter a model ID. Moeka defaults to `black-forest-labs/flux-schnell`; use the exact ID shown on the Replicate model page if you select another model.
