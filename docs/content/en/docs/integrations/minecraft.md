...
description: Run Moeka's local game agent on a trusted Minecraft server
---

The Minecraft integration uses Mineflayer to connect Moeka to a Minecraft server so the agent can receive context, perform in-game actions, and report state. It is intended for local development and maintenance. The current implementation is planned to migrate to a Fabric runtime, so avoid building new long-term features around it.
...
- Configure a working chat provider and model in Moeka, and prepare the OpenAI-compatible model settings used by the Minecraft agent.
...
Edit **integrations/minecraft/.env.local** and provide the required Minecraft server, Moeka, and model-service settings.

In the Desktop ver., open **Settings → Connection**. Show and copy the **Auth Token**. Then add these Moeka channel settings:
...
After startup, use the terminal output to verify that authentication to Moeka succeeds and that the agent connects to the Minecraft server. A missing or incorrect `AIRI_WS_TOKEN` prevents the module from registering with Moeka.
