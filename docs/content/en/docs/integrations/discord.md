---
title: Discord Bot
description: Run Moeka as a voice and messaging bot using a Discord application
---

The Discord bot connects to text and voice channels in a Discord server. Text replies come from the chat provider and model selected in Moeka.
...
- Configure a working chat provider and model in Moeka.

::: warning Credential security
Keep the Bot Token and Moeka Auth Token only in Moeka's local settings or the bot service's local **.env.local** file. Do not commit, screenshot, or share these credentials.
:::
...
`DISCORD_TOKEN` is an optional startup fallback. You can leave it empty and send the Bot Token from Moeka after the service connects. The service does not use `DISCORD_BOT_CLIENT_ID`, `OPENAI_MODEL`, `OPENAI_API_*`, or `ELEVENLABS_*`; Discord text replies use Moeka's active chat configuration.
...
The authenticated bot service receives the enabled state and token through Moeka's configuration channel. If the service is not running or its Moeka Auth Token is missing or incorrect, saving these fields alone does not start the Discord bot.
